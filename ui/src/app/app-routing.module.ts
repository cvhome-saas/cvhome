...
const routes: Routes = [
    {
        path: 'in', component: BaseComponent, canActivate: [authenticationGuard()], children: [
            ...
            {
                path: 'profile',
                loadChildren: () => import('./views/in/profile/profile.module').then(m => m.ProfileModule)
            },
            {
                path: 'settings',
                loadChildren: () => import('./views/in/settings/settings.module').then(m => m.SettingsModule)
            }
        ]
    },
