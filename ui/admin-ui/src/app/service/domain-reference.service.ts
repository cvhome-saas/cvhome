...
export class DomainReferenceService {
    ...
    save(domainReference: DomainReference): Observable<DomainReference> {
      return this.httpClient.post<DomainReference>("/domain-ownership/api/v1/domain-reference", domainReference)
    }

    getAllDomainReferencesOrders(): Observable<DomainReferenceOrder[]> {
      return this.httpClient.get<DomainReferenceOrder[]>("/domain-ownership/api/v1/domain-reference/get-all-domain-references")
    }

    orderHttps(id: number): Observable<DomainReferenceOrder> {
      return this.httpClient.post<DomainReferenceOrder>(`/domain-ownership/api/v1/domain-reference-acm/order?domainId=${id}`, null)
    }
}
