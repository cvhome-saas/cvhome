...
public class ExternalEventListener {
    ...
    @Bean
    public Consumer<Message<String>> sysEvents() {
        return event -> {
            try {
    ...
    public Consumer<Message<OrdersEvent>> logOrderEvents() {
        return event -> {
            ...
            try {
                orderEventHandler.handle(event.getPayload());
            } catch (Exception e) {
                log.error("error processing " + event.getPayload());
            }
        };
    ...
    @Bean
    public Consumer<Message<OrderCommand>> logOrderCommands() {
        return event -> {
            System.out.println("Received: order command: " + event.getPayload());
            try {
                orderCommandHandler.handle(event.getPayload());
            } catch (Exception e) {
                log.error("error processing " + event.getPayload());
            }
        };
