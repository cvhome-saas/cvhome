...
public class OrderServiceImpl extends SalesManagerEntityServiceImpl<Long, Order> implements OrderService {
	...
	/**
	 * <p>
	 * Method will be used to calculate Shopping cart total as well will update price for
	 * each line items.
	 * </p>
	...
	private OrderTotalSummary calculateOrder(OrderSummary summary, Customer customer, final StoreMerchantId store,
		...
		for (ShoppingCartItem item : summary.getProducts()) {
			...
			/*
			 * @TODO comment this for now as unnecessary FinalPrice finalPrice =
			 * productService.getProductPrice(store, item.getSku()); if (finalPrice !=
			 * null) { List<FinalPrice> otherPrices = finalPrice.getAdditionalPrices(); if
			 * (otherPrices != null) { for (FinalPrice price : otherPrices) { if
			 * (!price.isDefaultPrice()) { OrderTotal itemSubTotal =
			 ...
			 * itemSubTotal.setSortOrder(0); otherPricesTotals.put(
			 * price.getProductPrice().getCode(), itemSubTotal); }
			 *
			 * BigDecimal orderTotalValue = itemSubTotal.getValue(); if (orderTotalValue
			 * == null) { orderTotalValue = new BigDecimal(0); orderTotalValue.setScale(2,
			 * RoundingMode.HALF_UP); }
			 *
			 * orderTotalValue = orderTotalValue.add(price.getFinalPrice());
			 * itemSubTotal.setValue(orderTotalValue); if (OrderValueType.ONE_TIME .name()
			 * .equals(price.getProductPrice().getProductPriceType().name())) { subTotal =
		...
		// tax
		/*
		 * List<TaxItem> taxes = taxService.calculateTax(summary, customer, store,
		 * language); if(taxes!=null && taxes.size()>0) { BigDecimal totalTaxes = new
		 * BigDecimal(0); totalTaxes.setScale(2, RoundingMode.HALF_UP); int taxCount =
		 * 200; for(TaxItem tax : taxes) {
		 *
		 * OrderTotal taxLine = new OrderTotal();
		 * taxLine.setModule(Constants.OT_TAX_MODULE_CODE);
		 * taxLine.setOrderTotalType(OrderTotalType.TAX);
		 * taxLine.setOrderTotalCode(tax.getLabel()); taxLine.setSortOrder(taxCount);
		 * taxLine.setTitle(Constants.OT_TAX_MODULE_CODE);
		 * taxLine.setText(tax.getLabel()); taxLine.setValue(tax.getItemPrice());
		 *
		 * totalTaxes = totalTaxes.add(tax.getItemPrice()); orderTotals.add(taxLine);
