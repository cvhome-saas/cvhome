...
public interface ProductSnapshotService {
    ...
    /**
     * A cart's lines priced for a read: inventory live for every line, the catalogue only for a line that carries no
     * snapshot it may still trust ({@link CartLine#remembers}). The lines it had to ask the catalogue about are
     * refreshed in place, so the caller's write keeps the answer.
     */
