package com.asrevo.cvhome.store.core.repositories.catalog.product.variation;
...
public interface PageableProductVariationRepository extends PagingAndSortingRepository<ProductVariation, Long> {
    ...
    @Query(value = """
            select distinct p from ProductVariation p join fetch p.merchantStore pm
            ...
            countQuery = """
                    select count(p) from ProductVariation p join p.merchantStore pm where pm.id = ?1 and (?2 is null or p.code like %?2%)""")
