package com.asrevo.cvhome.store.core.repositories.payments;
...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query(
            "select t from Transaction t join fetch t.order to left join fetch to.orderAttributes"
                + " toa left join fetch to.orderProducts too left join fetch to.orderTotal toot"
                + " left join fetch to.orderHistory tood where to is not null and t.transactionDate"
                + " BETWEEN :from AND :to")
