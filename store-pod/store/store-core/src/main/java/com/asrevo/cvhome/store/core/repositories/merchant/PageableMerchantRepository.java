...
public interface PageableMerchantRepository
        extends PagingAndSortingRepository<MerchantStore, Long> {
    ...
    @Query(
            value =
                    """
            select distinct m from MerchantStore m
            ...
            countQuery =
                    """
                    select count(distinct m) from MerchantStore m
                    join m.parent mp
                    where mp.code = ?1""")
    ...
    @Query(
            ...
            left join fetch m.zone mz
            left join fetch m.defaultLanguage md
            left join fetch m.languages ml
    ...
    @Query(
            ...
            countQuery =
                    ...
                    select count(distinct m) from MerchantStore m
                    join m.parent
                    where m.retailer = true
                    and (?1 is null or m.storename like %?1%)""")
    ...
    @Query(
            ...
            countQuery =
                ...
                select count(distinct m) from MerchantStore m
                left join m.parent mp
                where mp.code = ?1 or m.code = ?1 and (?2 is null or (m.storename like %?2% or
     mp.storename like %?2%))""")
    ...
    @Query(
            value =
                    """
                select * from MERCHANT_STORE m where (m.STORE_CODE = ?1 or (?2 is null or
     m.PARENT_ID = ?2))
                and (?3 is null or m.STORE_NAME like %?3%)""",
            countQuery =
                    """
                select count(*) from {h-schema}MERCHANT_STORE m where (m.STORE_CODE = ?1 or (?2 is
     null or m.PARENT_ID = ?2)) and (?3 is null or m.STORE_NAME like %?3%)""",
            nativeQuery = true)
    Page<MerchantStore> listByGroup(
            String storeCode, Integer id, String storeName, Pageable pageable);
}
