package com.asrevo.cvhome.store.core.repositories.customer.attribute;
...
public interface CustomerAttributeRepository extends JpaRepository<CustomerAttribute, Long> {
    ...
    @Query(
            "select a from CustomerAttribute a left join fetch a.customerOption aco left join fetch"
                    + " a.customerOptionValue acov left join fetch aco.descriptions acod left join"
                    + " fetch acov.descriptions acovd where a.id = ?1")
    CustomerAttribute findOne(Long id);

    @Query(
            "select a from CustomerAttribute a join fetch a.customer ac left join fetch"
                + " a.customerOption aco join fetch aco.merchantStore acom left join fetch"
                + " a.customerOptionValue acov left join fetch aco.descriptions acod left join"
                + " fetch acov.descriptions acovd where acom.id = ?1 and ac.id = ?2 and aco.id ="
                + " ?3")
    ...
    @Query(
            "select a from CustomerAttribute a join fetch a.customer ac left join fetch"
                    + " a.customerOption aco join fetch aco.merchantStore acom left join fetch"
                    + " a.customerOptionValue acov left join fetch aco.descriptions acod left join"
                    + " fetch acov.descriptions acovd where acom.id = ?1 and aco.id = ?2")
    ...
    @Query(
            "select distinct a from CustomerAttribute a join fetch a.customer ac left join fetch"
                    + " a.customerOption aco join fetch aco.merchantStore acom left join fetch"
                    + " a.customerOptionValue acov left join fetch aco.descriptions acod left join"
                    + " fetch acov.descriptions acovd where acom.id = ?1 and ac.id = ?2")
    ...
    @Query(
            "select a from CustomerAttribute a join fetch a.customer ac left join fetch"
                    + " a.customerOption aco join fetch aco.merchantStore acom left join fetch"
                    + " a.customerOptionValue acov left join fetch aco.descriptions acod left join"
                    + " fetch acov.descriptions acovd where acom.id = ?1 and acov.id = ?2")
