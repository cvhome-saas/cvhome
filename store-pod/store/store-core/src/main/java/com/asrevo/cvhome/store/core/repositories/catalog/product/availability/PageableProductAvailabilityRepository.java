...
public interface PageableProductAvailabilityRepository extends PagingAndSortingRepository<ProductAvailability, Long> {
    ...
    @Query(value = """
            select distinct p from ProductAvailability p left join fetch p.merchantStore pm
            ...
            and (?3 is null or pm.code like %?3%)""", countQuery = """
            select  count(p) from ProductAvailability p
            join p.merchantStore pm join p.prices pp join pp.descriptions ppd
            join p.merchantStore pm2 join p.product ppr join ppr.merchantStore pprm
    ...
    @Query(value = """
            ...
            where pm.id=?1\s""", countQuery = """
            select  count(p) from ProductAvailability p
            join p.merchantStore pm where pm.id=?1\s""")
    ...
    @Query(value = """
            ...
            where ppr.id=?1 and pm.code=?2""", countQuery = """
            select  count(p) from ProductAvailability p
    ...
    @Query(value = """
            ...
            and pm.code=?2""", countQuery = """
            select  count(p) from ProductAvailability p join p.merchantStore pm
    ...
    @Query(value = """
            ...
            where ppr.sku=?1 or ppi.sku=?1\s""")
