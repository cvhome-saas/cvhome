package com.asrevo.cvhome.store.core.repositories.catalog.product.manufacturer;
...
public interface ManufacturerRepository extends JpaRepository<Manufacturer, Long> {
    ...
    @Query(
            """
            select count(distinct m) from Manufacturer as m where m.merchantStore.id=?1""")
    ...
    @Query(
            ...
            and pc.id IN (
                ...
                where c.lineage like %?2% and pmd.language.id = ?3
            )""")
    List<Manufacturer> findByProductInCategoryId(
