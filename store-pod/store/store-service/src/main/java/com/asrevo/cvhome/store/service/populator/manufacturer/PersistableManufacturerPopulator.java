...
public class PersistableManufacturerPopulator
        extends AbstractDataPopulator<PersistableManufacturer, Manufacturer> {
    ...
    public Manufacturer populate(
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(source.getDescriptions())) {
                Set<
                                com.asrevo.cvhome.store.core.entity.catalog.product.manufacturer
                ...
                for (ManufacturerDescription description : source.getDescriptions()) {
                    com.asrevo.cvhome.store.core.entity.catalog.product.manufacturer
                                    .ManufacturerDescription
                            desc =
                                    new com.asrevo.cvhome.store.core.entity.catalog.product
                                            .manufacturer.ManufacturerDescription();
                    if (desc.getId() != null && desc.getId() > 0) {
                        desc.setId(description.getId());
                    }
                    if (target.getDescriptions() != null) {
                        for (com.asrevo.cvhome.store.core.entity.catalog.product.manufacturer
                                        .ManufacturerDescription
                                d : target.getDescriptions()) {
                            if (d.getLanguage().getCode().equals(description.getLanguage())
                                    || desc.getId() != null
