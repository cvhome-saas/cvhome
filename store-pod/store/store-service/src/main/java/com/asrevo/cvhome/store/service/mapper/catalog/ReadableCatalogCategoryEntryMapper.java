...
@Component
public class ReadableCatalogCategoryEntryMapper
        implements Mapper<CatalogCategoryEntry, ReadableCatalogCategoryEntry> {

    private final ReadableCategoryMapper readableCategoryMapper;

    public ReadableCatalogCategoryEntryMapper(
            ReadableCategoryMapper readableCategoryMapper, ImageFilePath imageUtils) {
        this.readableCategoryMapper = readableCategoryMapper;
    }

    @Override
    public ReadableCatalogCategoryEntry convert(
            CatalogCategoryEntry source, MerchantStore store, Language language) {
        ReadableCatalogCategoryEntry destination = new ReadableCatalogCategoryEntry();
        return merge(source, destination, store, language);
    }

    @Override
    public ReadableCatalogCategoryEntry merge(
            CatalogCategoryEntry source,
            ReadableCatalogCategoryEntry destination,
            MerchantStore store,
            Language language) {
        ReadableCatalogCategoryEntry convertedDestination =
                Optional.ofNullable(destination).orElse(new ReadableCatalogCategoryEntry());
        ...
        try {
            ...
            // readableProductPopulator.populate(source.getProduct(), store, language);
