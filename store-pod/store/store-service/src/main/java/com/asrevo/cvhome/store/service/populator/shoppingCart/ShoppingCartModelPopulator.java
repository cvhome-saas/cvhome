...
public class ShoppingCartModelPopulator
    ...
    @Override
    public ShoppingCart populate(
            ShoppingCartData shoppingCart,
            ShoppingCart cartMdel,
            ...
            if (items != null && !items.isEmpty()) {
                for (ShoppingCartItem item : items) {
                    ...
                    Set<com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem>
                            cartItems = cartMdel.getLineItems();
                    if (cartItems != null && !cartItems.isEmpty()) {
                        ...
                        for (com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem
                            ...
                            if (dbItem.getId().longValue() == item.getId()) {
                                ...
                                // compare attributes
                                Set<ShoppingCartAttributeItem> attributes = dbItem.getAttributes();
                                Set<ShoppingCartAttributeItem> newAttributes = new HashSet<>();
                                List<ShoppingCartAttribute> cartAttributes =
                                        item.getShoppingCartAttributes();
                                if (!CollectionUtils.isEmpty(cartAttributes)) {
                                    for (ShoppingCartAttribute attribute : cartAttributes) {
                                        for (ShoppingCartAttributeItem dbAttribute : attributes) {
                                            if (dbAttribute.getId().longValue()
                                                    == attribute.getId()) {
                        ...
                        Set<com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem>
                                lineItems = cartMdel.getLineItems();
                        if (lineItems == null) {
    ...
    private com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem createCartItem(
            ShoppingCart cart, ShoppingCartItem shoppingCartItem, MerchantStore store)
            throws Exception {
        ...
        com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem item =
                new com.asrevo.cvhome.store.core.entity.shoppingcart.ShoppingCartItem(
                        cart, product);
