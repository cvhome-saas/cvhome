...
public class PersistableCategoryPopulator
        extends AbstractDataPopulator<PersistableCategory, Category> {

    private final CategoryService categoryService;
    private final LanguageService languageService;

    public PersistableCategoryPopulator(
    ...
    @Override
    public Category populate(
            PersistableCategory source, Category target, MerchantStore store, Language language)
        ...
        try {
            ...
            if (source.getParent() == null
                    || (StringUtils.isBlank(source.getParent().getCode()))
                ...
                if (!StringUtils.isBlank(source.getParent().getCode())) {
                    parent =
                            categoryService.getByCode(
                                    store.getCode(), source.getParent().getCode());
                ...
                if (parent != null
                        && parent.getMerchantStore().getId().intValue()
                                != store.getId().intValue()) {
                    throw new ConversionException(
                            "Store id does not belong to specified parent id");
                }
            ...
            if (!CollectionUtils.isEmpty(source.getDescriptions())) {
                Set<com.asrevo.cvhome.store.core.entity.catalog.category.CategoryDescription>
                        descriptions = new HashSet<>();
                if (CollectionUtils.isNotEmpty(target.getDescriptions())) {
                    for (com.asrevo.cvhome.store.core.entity.catalog.category.CategoryDescription
                            description : target.getDescriptions()) {
                        for (CategoryDescription d : source.getDescriptions()) {
                            if (StringUtils.isBlank(d.getLanguage())) {
                                throw new ConversionException(
                                        "Source category description has no language");
                            }
                    ...
                    for (CategoryDescription d : source.getDescriptions()) {
                        com.asrevo.cvhome.store.core.entity.catalog.category.CategoryDescription t =
                                new com.asrevo.cvhome.store.core.entity.catalog.category
                                        .CategoryDescription();
    ...
    private com.asrevo.cvhome.store.core.entity.catalog.category.CategoryDescription
            buildDescription(
                    CategoryDescription source,
                    com.asrevo.cvhome.store.core.entity.catalog.category.CategoryDescription target)
                    throws Exception {
