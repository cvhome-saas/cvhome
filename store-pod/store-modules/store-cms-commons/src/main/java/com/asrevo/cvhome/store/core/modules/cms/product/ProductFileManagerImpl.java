	...
	public void addProductImage(CmsProductImage productImage, ImageContentFile contentImage) throws ServiceException {
		...
		try {
			...
			if (!StringUtils.isBlank(slargeImageHeight) && !StringUtils.isBlank(slargeImageWidth)) {
				...
				// do not keep a large image for now, just take care of the regular image
				// and a
				// small image
				...
				try (FileInputStream isLarge = new FileInputStream(tempLarge)) {
					 ...
					 * StringBuilder().append(productImage.getProduct().getId()).append(
					 * "tmpSmall").toString(), "." + extension );
