...
@Configuration
@ConditionalOnProperty(prefix = "mock", name = "auth.enabled")
public class MockAuthServerConfig {
    ...
    @Bean
    public CommandLineRunner authRunner(@Qualifier("authMockServer") MockServerContainer authMockServer) {
        return args -> {
            MockServerClient mockServerClient = new MockServerClient(authMockServer.getHost(), authMockServer.getServerPort());
            mockServerClient
                    .when(
                            request()
//            .withMethod("POST")
                                    .withPath("/realms/cvhome/protocol/openid-connect/token")
//            .withContentType(new MediaType("application","x-www-form-urlencoded","UTF-8",null))
//            .withBody("client_id=microservice-app&client_secret=r1nuVn4HefusLGwHIeRxbdOzdTZLEMvQ&grant_type=password&username=microservice-gateway&password=microservice-gateway")
                                    .withBody("grant_type=password&username=microservice-gateway&password=microservice-gateway")
                    )
                    .respond(
                            response()
