...
@Slf4j
public class RedirectingServerAuthenticationSuccessHandler
    ...
    public Mono<Void> onAuthenticationSuccess(
        ...
        return exchange.getSession()
                .flatMap(
                        webSession -> {
                            String stateFromCallback =
                                    exchange.getRequest().getQueryParams().getFirst("state");

                            if (!StringUtils.hasText(stateFromCallback)
                                    && authentication instanceof OAuth2AuthenticationToken) {
                                log.warn(
                                        "State parameter not found directly in callback query"
                                            + " params. This might affect redirectTo retrieval.");
                            }

                            if (StringUtils.hasText(stateFromCallback)) {
                                String sessionKey =
                                        CapturingServerOAuth2AuthorizationRequestResolver
                                                        .CAPTURED_PARAMETERS_SESSION_KEY_PREFIX
                                                + stateFromCallback;
                                Object capturedParamsObject =
                                        webSession.getAttributes().get(sessionKey);

                                if (capturedParamsObject instanceof Map) {
                                    @SuppressWarnings("unchecked")
                                    Map<String, String> capturedParams =
                                            (Map<String, String>) capturedParamsObject;
                                    String redirectTo = capturedParams.get("redirectTo");

                                    log.info(
                                            "Retrieved captured parameters from session (key: {}):"
                                                    + " {}",
                                            sessionKey,
                                            capturedParams);
                                    ...
                                    log.debug(
                                            "Removed captured parameters from session with key: {}",
                                            sessionKey);

                                    if (StringUtils.hasText(redirectTo)) {
                                        if (isValidRedirect(redirectTo)) {
                                            log.info(
                                                    "Valid 'redirectTo' parameter found: '{}'."
                                                            + " Redirecting.",
                                                    redirectTo);
                                            return this.redirectStrategy.sendRedirect(
                                                    exchange, URI.create(redirectTo));
                                        } else {
                                            log.warn(
                                                    "Invalid or disallowed 'redirectTo' parameter:"
                                                            + " '{}'. Falling back to default.",
                                                    redirectTo);
                                        }
                                    } else {
                                        log.debug(
                                                "'redirectTo' parameter not found in captured"
                                                        + " params or is empty. Falling back to"
                                                        + " default.");
                                    }
                                } else if (capturedParamsObject != null) {
                                    log.warn(
                                            "Captured parameters in session (key: {}) were not of"
                                                    + " expected Map type: {}. Falling back to"
                                                    + " default.",
                                            sessionKey,
                                            capturedParamsObject.getClass().getName());
                                    webSession
                                            .getAttributes()
                                            .remove(sessionKey); // Clean up mismatched type
                                } else {
                                    log.debug(
                                            "No captured parameters found in session for key: {}."
                                                    + " Falling back to default.",
                                            sessionKey);
                                }
                            } else {
                                log.warn(
                                        "State parameter was missing or empty in OAuth2 callback."
                                                + " Cannot retrieve 'redirectTo'. Falling back to"
                                                + " default.");
                            }
    ...
    private boolean isValidRedirect(String url) {
        ...
        try {
            ...
            if (!redirectUri.isAbsolute()) {
                ...
                if (url.startsWith("//")) {
                    log.warn(
                            "Disallowing scheme-relative URL that doesn't match app base: {}", url);
                    return false;
                }
            ...
            // If URL is not absolute and has no host (shouldn't happen with new URI(url) if valid)
            return false;
