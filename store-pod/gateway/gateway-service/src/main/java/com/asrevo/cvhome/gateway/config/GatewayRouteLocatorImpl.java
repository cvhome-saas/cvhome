...
public class GatewayRouteLocatorImpl implements RouteLocator {
	...
	public Flux<Route> getRoutes() {
		...
		Predicate<ServerWebExchange> merchantUiHostPredicate = hostRoutePredicate
			.apply(config -> config.setHost(Set.of("merchant-ui." + storeCoreGatewayDomain)));
		Predicate<ServerWebExchange> podAuthHostPredicate = hostRoutePredicate
			.apply(config -> config.setHost(Set.of("pod-auth." + storeCoreGatewayDomain)));
		...
		backendServices.forEach(bs -> routes
			.route(r -> r.path("/" + bs + "/**")
				...
				.uri("lb://" + bs))
			.route(r -> r.path("/store-pod-gateway/" + bs + "/**")
				.filters(f -> f.stripPrefix(2).tokenRelay().preserveHostHeader())
				.uri("lb://" + bs)));

		routes.route(r -> r.predicate(notBackendService).and().predicate(podAuthHostPredicate).uri("lb://pod-auth"))
			.route(r -> r.predicate(notBackendService).and().predicate(merchantUiHostPredicate).uri("lb://merchant-ui"))
