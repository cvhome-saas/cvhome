package com.asrevo.cvhome.content.model.legacy;

import java.io.Serial;
import java.util.List;

import com.asrevo.cvhome.commons.domain.Entity;

import lombok.Getter;
import lombok.Setter;

/**
 * Legacy {@code GET /api/v1/content/pages[/name/{name}]} item shape: {@code {id, code, visible, linkToMenu,
 * contentType:"PAGE", path, description, descriptions}}.
 */
@Getter
@Setter
public class LegacyContentPage extends Entity {

    @Serial
    private static final long serialVersionUID = 1L;

    private String code;

    private boolean visible;

    private boolean linkToMenu;

    private String contentType = "PAGE";

    private String path;

    private LegacyDescription description;

    private List<LegacyDescription> descriptions;

}
