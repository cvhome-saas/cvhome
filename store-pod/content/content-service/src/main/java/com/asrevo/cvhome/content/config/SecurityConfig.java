...
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.authorizeHttpRequests(
                        it ->
                                it.requestMatchers("api/v1/test/sign")
                                        .permitAll()
    ...
    public JwtAuthenticationConverter converter() {
        ...
        jwtAuthenticationConverter.setJwtGrantedAuthoritiesConverter(
                keyClockJwtGrantedAuthoritiesConverter);
    ...
    public AccessEvaluator accessEvaluator(
        ...
        return new AccessEvaluatorImpl(
                new StoreSecurityServiceImpl(
                        (it) -> {
                            ReadableMerchantStore merchantStore =
                                    externalMerchantStoreService.getStore(
                                            new StoreMerchantId(it.getId().toString()));
