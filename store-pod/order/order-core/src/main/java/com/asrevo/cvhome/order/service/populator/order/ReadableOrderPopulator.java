...
import com.asrevo.cvhome.order.entity.order.OrderTotalType;
...
@Component
public class ReadableOrderPopulator
        extends AbstractDataPopulator<Order, StoreMerchantId, ReadableOrder> {

    @Override
    public ReadableOrder populate(
            Order source, ReadableOrder target, StoreMerchantId store, LanguageCode language) {
        ...
        com.asrevo.cvhome.order.model.order.total.OrderTotal taxTotal = null;
        com.asrevo.cvhome.order.model.order.total.OrderTotal shippingTotal = null;
        ...
        List<com.asrevo.cvhome.order.model.order.total.OrderTotal> totals = new ArrayList<>();
        for (OrderTotal t : source.getOrderTotal()) {
            ...
            if (t.getOrderTotalType().name().equals(OrderTotalType.TOTAL.name())) {
                com.asrevo.cvhome.order.model.order.total.OrderTotal totalTotal = createTotal(t);
                ...
                com.asrevo.cvhome.order.model.order.total.OrderTotal subTotal = createTotal(t);
                totals.add(subTotal);

            } else {
                com.asrevo.cvhome.order.model.order.total.OrderTotal otherTotal = createTotal(t);
    ...
    private com.asrevo.cvhome.order.model.order.total.OrderTotal createTotal(OrderTotal t) {
        com.asrevo.cvhome.order.model.order.total.OrderTotal totalTotal =
                new com.asrevo.cvhome.order.model.order.total.OrderTotal();
