...
public class ProductVariantFacadeImpl implements ProductVariantFacade {
    ...
    public ProductVariantFacadeImpl(
            ...
            @Qualifier("productFacade") ProductFacade productFacade,
    ...
    public ReadableProductVariant get(
            Long instanceId, Long productId, StoreMerchantId store, LanguageCode language) {
        Optional<ProductVariant> productVariant =
                this.getproductVariant(instanceId, productId, store);
    ...
    public boolean exists(
            String sku, StoreMerchantId store, Long productId, LanguageCode language) {
    ...
    public Long create(
        ...
        if (productVariant.getVariation() != null
                && productVariant.getVariation() > 0
                && productVariant.getVariationValue() != null
                && productVariant.getVariationValue() > 0) {

            List<ProductVariation> variations =
                    productVariationService.getByIds(
                            Arrays.asList(
                                    productVariant.getVariation(),
                                    productVariant.getVariationValue()),
                            store);

            boolean differentOption =
                    variations.stream().map(i -> i.getProductOption().getCode()).distinct().count()
                            > 1;

            if (!differentOption) {
                throw new ConstraintException(
                        "Product option of instance.variant and instance.variantValue must be"
                                + " different");
            }
        }
        ...
        ProductVariant variant =
                persistableProductVariantMapper.convert(productVariant, store, language);
    ...
    public void update(
        ...
        Optional<ProductVariant> instanceModel =
                this.getproductVariant(instanceId, productId, store);
        ...
        ProductVariant mergedModel =
                persistableProductVariantMapper.merge(
                        productVariant, instanceModel.get(), store, language);
        productVariantService.saveProductVariant(mergedModel);
    }

    private Optional<ProductVariant> getproductVariant(
            Long id, Long productId, StoreMerchantId store) {
    ...
    public void delete(Long productVariant, Long productId, StoreMerchantId store) {
        ...
        Optional<ProductVariant> instanceModel =
                this.getproductVariant(productVariant, productId, store);
        if (instanceModel.isEmpty()) {
            throw new ResourceNotFoundException(
                    "productVariant with id ["
                            + productVariant
        ...
        try {
            ...
            throw new ServiceRuntimeException(
                    "Cannot delete product instance ["
                            + productVariant
                            + "]  for store ["
                            + store
                            + "] and productId ["
