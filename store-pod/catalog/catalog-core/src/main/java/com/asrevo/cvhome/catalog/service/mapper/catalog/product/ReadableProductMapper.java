...
import com.asrevo.cvhome.catalog.model.product.attribute.*;
...
public class ReadableProductMapper implements Mapper<Product, ReadableProduct> {
    ...
    public ReadableProduct merge(
            ...
            if (!CollectionUtils.isEmpty(attributes)) {
                ...
                for (ProductAttribute attribute : attributes) {
                    ...
                    if (attribute.isAttributeDisplayOnly()) { // read only attribute = property
                        ...
                        com.asrevo.cvhome.catalog.model.product.attribute
                                        .ProductOptionValueDescription
                                valueDescription =
                                        new com.asrevo.cvhome.catalog.model.product.attribute
                                                .ProductOptionValueDescription();
                        ...
                        if (podescriptions != null && !podescriptions.isEmpty()) {
                            ...
                            if (podescriptions.size() > 1) {
                                for (ProductOptionValueDescription optionValueDescription :
                                        podescriptions) {
                                    if (Objects.equals(
                                            optionValueDescription.getLanguageCode(), language)) {
        ...
        if (!CollectionUtils.isEmpty(source.getVariants())) {
            ...
            defaultInstance =
                    instances.stream()
                            .filter(
                                    com.asrevo.cvhome.catalog.model.product.product.variant
                                                    .ProductVariant
                                            ::isDefaultSelection)
                            .findAny()
                            .orElse(null);
        ...
        for (ProductAvailability a : source.getAvailabilities()) {
            ...
            destination.setQuantity(
                    Optional.ofNullable(availability.getProductQuantity()).orElse(1));
            destination.setQuantityOrderMaximum(
                    Optional.ofNullable(availability.getProductQuantityOrderMax()).orElse(1));
            destination.setQuantityOrderMinimum(
                    Optional.ofNullable(availability.getProductQuantityOrderMin()).orElse(1));
                        ...
                        if (pr.isPresent() && language != null) {
                            ...
                            Optional<ProductPriceDescription> d =
                                    pr.get().getDescriptions().stream()
                                            ...
                                            .findFirst();
                            if (d.isPresent()) {
                                com.asrevo.cvhome.catalog.model.product.ProductPriceDescription
                                        priceDescription =
                                                new com.asrevo.cvhome.catalog.model.product
    ...
    private ReadableProductProperty createProperty(
            ProductAttribute productAttribute, LanguageCode language) {
        ...
        if (descriptions != null && !descriptions.isEmpty()) {
            for (ProductOptionDescription optionDescription : descriptions) {
                com.asrevo.cvhome.catalog.model.product.attribute.ProductOptionValueDescription
                        productOptionValueDescription =
                                new com.asrevo.cvhome.catalog.model.product.attribute
                                        .ProductOptionValueDescription();
    ...
    private Optional<ReadableProductOptionValue> optionValue(
            ProductOptionValue optionValue, StoreMerchantId store, LanguageCode language) {
