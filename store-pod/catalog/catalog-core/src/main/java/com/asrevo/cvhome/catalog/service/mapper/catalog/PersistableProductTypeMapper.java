...
public class PersistableProductTypeMapper implements Mapper<PersistableProductType, ProductType> {
    ...
    public ProductType convert(
            PersistableProductType source, StoreMerchantId store, LanguageCode language) {
    ...
    private ProductType type(PersistableProductType type, ProductType destination) {
        ...
        List<com.asrevo.cvhome.catalog.entity.product.type.ProductTypeDescription> descriptions =
                new ArrayList<>();
        if (!CollectionUtils.isEmpty(type.getDescriptions())) {

            for (ProductTypeDescription d : type.getDescriptions()) {
                com.asrevo.cvhome.catalog.entity.product.type.ProductTypeDescription desc =
                        typeDescription(d, destination, d.getLanguage());
    ...
    private com.asrevo.cvhome.catalog.entity.product.type.ProductTypeDescription
            findAppropriateDescription(ProductType typeModel, LanguageCode lang) {
        java.util.Optional<com.asrevo.cvhome.catalog.entity.product.type.ProductTypeDescription> d =
