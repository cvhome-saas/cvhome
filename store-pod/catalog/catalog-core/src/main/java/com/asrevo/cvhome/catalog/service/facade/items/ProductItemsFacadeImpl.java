...
public class ProductItemsFacadeImpl implements ProductItemsFacade {
	...
	public ProductItemsFacadeImpl(ProductService productService, PricingService pricingService,
			ImageFilePath imageUtils, ProductRelationshipService productRelationshipService,
	...
	private ReadableProductList getReadableProductGroup(String group, StoreMerchantId store, LanguageCode language,
		...
		if (productGroup != null && productGroup.isActive()) {
			...
			List<ReadableProduct> productList = groups.stream().map(it -> {
				try {
					return populator.populate(it.getRelatedProduct(), new ReadableProduct(), store, language);
				}
				catch (Exception e) {
					return null;
				}
			}).filter(Objects::nonNull).sorted(Comparator.comparing(ReadableProduct::getSortOrder)).toList();
	...
	@Override
	public ReadableProductList listTinyProductsGroup(String group, StoreMerchantId store, LanguageCode language) {
	...
	@Override
	public ReadableProductList listMinimalProductsGroup(String group, StoreMerchantId store, LanguageCode language) {
		ReadableMinimalProductPopulator populator = new ReadableMinimalProductPopulator(pricingService, imageUtils,
				externalMerchantStoreService);
		return getReadableProductGroup(group, store, language, populator);
	}

	@Override
	public void addItemToGroup(Product product, String group, StoreMerchantId store, LanguageCode language) {
		...
		existList = productRelationshipService.getByGroup(store, group)
			.stream()
			.filter(prod -> prod.getRelatedProduct() != null
					&& (product.getId().equals(prod.getRelatedProduct().getId())))
			.toList();
	...
	@Override
	public void removeItemFromGroup(Product product, String group, StoreMerchantId store, LanguageCode language) {
	...
	@Override
	public ReadableProductList relatedTinyProducts(Product product, StoreMerchantId merchantStore,
	...
	@Override
	public ReadableProductList relatedMinimalProducts(Product product, StoreMerchantId merchantStore,
			LanguageCode language) {
		ReadableMinimalProductPopulator populator = new ReadableMinimalProductPopulator(pricingService, imageUtils,
				externalMerchantStoreService);
		return getReadableRelatedProductGroup(product, merchantStore, language, populator);
	...
	private ReadableProductList getReadableRelatedProductGroup(Product product, StoreMerchantId merchantStore,
			LanguageCode language, AbstractDataPopulator<Product, StoreMerchantId, ReadableProduct> populator) {
		List<ProductRelationship> groups = productRelationshipService.getByType(merchantStore, product, "RELATED_ITEM",
				language);
		...
		List<ReadableProduct> productList = groups.stream().map(it -> {
			try {
				return populator.populate(it.getProduct(), new ReadableProduct(), merchantStore, language);
			}
			catch (Exception e) {
				return null;
			}
	...
	@SneakyThrows
	@Override
	public void addItemToRelatedProduct(Product product, Product related, StoreMerchantId store,
			LanguageCode language) {
		Assert.notNull(product, "Product must not be null");
		...
		List<ProductRelationship> existList;
		existList = productRelationshipService.getByType(store, "RELATED_ITEM", product, related, language)
			.stream()
			.toList();
	...
	@Override
	public void removeItemFromRelated(Product product, Product related, StoreMerchantId store, LanguageCode language) {
	...
	public ReadableProductGroupList listProductGroups(StoreMerchantId store, LanguageCode language) {
		...
		return productRelationshipService.getGroups(store).stream().map(it -> {
