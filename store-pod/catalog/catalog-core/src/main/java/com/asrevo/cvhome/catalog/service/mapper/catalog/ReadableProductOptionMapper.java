...
@Component
public class ReadableProductOptionMapper
        implements Mapper<ProductOption, ReadableProductOptionEntity> {

    @Override
    public ReadableProductOptionEntity convert(
            ProductOption source, StoreMerchantId store, LanguageCode language) {
        ReadableProductOptionEntity destination = new ReadableProductOptionEntity();
        return merge(source, destination, store, language);
    }

    @Override
    public ReadableProductOptionEntity merge(
            ProductOption source,
            ReadableProductOptionEntity destination,
            StoreMerchantId store,
            LanguageCode language) {
        ReadableProductOptionEntity readableProductOption = new ReadableProductOptionEntity();
        if (language == null) {
            List<com.asrevo.cvhome.catalog.model.product.attribute.ProductOptionDescription>
                    descriptions = new ArrayList<>();
            for (ProductOptionDescription desc : source.getDescriptions()) {
                com.asrevo.cvhome.catalog.model.product.attribute.ProductOptionDescription d =
            ...
            if (!CollectionUtils.isEmpty(source.getDescriptions())) {
                for (ProductOptionDescription desc : source.getDescriptions()) {
                    if (desc != null && desc.getLanguageCode().equals(language)) {
                        com.asrevo.cvhome.catalog.model.product.attribute.ProductOptionDescription
