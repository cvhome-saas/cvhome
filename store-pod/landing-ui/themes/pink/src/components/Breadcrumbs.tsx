...
/** The running head: where in the issue this page sits, set small in the cover face. */
export function Breadcrumbs({items, className}: { items: Crumb[]; className?: string }) {
    if (items.length === 0) return null;
    const last = items[items.length - 1];
    return (
        <Breadcrumb className={cn('cover-line', className)}>
            <BreadcrumbList className="gap-1.5 text-[length:inherit] sm:gap-1.5">
                {items.slice(0, -1).map(c => (
                    <Fragment key={c.id}>
                        <BreadcrumbItem>
                            <BreadcrumbLink asChild className="hover:text-foreground"><Link prefetch={false} href={c.href}>{c.name}</Link></BreadcrumbLink>
                        </BreadcrumbItem>
