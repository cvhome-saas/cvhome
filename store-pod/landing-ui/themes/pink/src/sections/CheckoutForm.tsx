    ...
    return (
        <>
            ...
            <form onSubmit={f.handleSubmit(f.onSubmit)} noValidate className="flex flex-col gap-6">
                <h2 className="hair display border-b-2 pb-3 text-2xl">{t('FORM_TITLE')}</h2>
                ...
                {f.agreement && (
                    <div className="flex items-start gap-2">
                        ...
                        <div className="flex flex-col gap-1">
                            <Label htmlFor="isAgree" className="text-sm font-medium normal-case tracking-normal">
                                {t('AGREEMENT')}{' '}
                ...
                <Button type="submit" size="lg" className="w-full sm:w-auto sm:self-start sm:px-10">{t('PLACE_ORDER')}</Button>
            </form>
        </>
    );
}
...
function SuccessDialog({order, open, onOpenChange}: { order: Order | undefined; open: boolean; onOpenChange: (o: boolean) => void }) {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="hair rounded-none border-2">
...
function LoginRequiredDialog({open, onOpenChange, login}: { open: boolean; onOpenChange: (o: boolean) => void; login: () => void }) {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="hair rounded-none border-2">
                <AlertDialogHeader className="items-center text-center">
                    <HelpCircleIcon className="size-12 text-primary" aria-hidden/>
