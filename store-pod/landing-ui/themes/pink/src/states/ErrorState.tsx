...
export function ErrorState({error, reset}: { error: Error & { digest?: string }; reset: () => void }) {
    ...
    return (
        <PageShell width="narrow" className="py-section">
            <ErrorBlock className="issue-block" icon={<AlertTriangleIcon/>} title={t('ERROR_TITLE')}
                ...
                <p>{body}</p>
                {error.digest && <p className="figure mt-1 text-xs">{t('ERROR_REFERENCE', {digest: error.digest})}</p>}
            </ErrorBlock>
        </PageShell>
    );
}
