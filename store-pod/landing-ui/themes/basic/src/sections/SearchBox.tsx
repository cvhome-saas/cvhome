...
export function SearchBox({storeContext, capabilities, className}: { storeContext: StoreContext; capabilities: SearchCapabilities; className?: string }) {
    ...
    return (
        <div className={cn('relative', className)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setFocused(false); }}>
            <form role="search" onSubmit={submit}>
                <label className="relative block">
                    ...
                    <Input type="search" role="combobox" aria-expanded={open} aria-controls={combobox.listboxId} aria-autocomplete="list" {...combobox.inputProps} value={query}
                           ...
                           onChange={e => { setQuery(e.target.value); setFocused(true); }} onFocus={() => setFocused(true)} onClick={() => setFocused(true)} className="h-10 w-full bg-muted/50 ps-9 pe-9 shadow-none"/>
                    {query && (
                        <Button type="button" variant="ghost" size="icon-sm" aria-label={t('LABEL')} onClick={clear} className="absolute end-1 top-1/2 -translate-y-1/2">
                            <XIcon/>
                        </Button>
                    )}
                </label>
            ...
            {open && (
                <div id={combobox.listboxId} role="listbox" className="absolute start-0 end-0 top-full z-50 mt-1 rounded-overlay border bg-popover py-1 text-popover-foreground shadow-overlay">
                    {!capabilities.text && <p className="border-b px-3 py-1.5 text-xs uppercase tracking-wide text-muted-foreground">{t('SUGGESTIONS_ONLY')}</p>}
                    ...
                    {hits.map((h, i) => (
                        <Link key={h.id} prefetch={false} href={h.href} {...combobox.optionProps(i)} onClick={clear}
                              className="flex items-center justify-between gap-3 px-3 py-2 text-sm hover:bg-accent focus:bg-accent data-[active=true]:bg-accent">
                            <span className="truncate"><bdi dir="auto">{h.title}</bdi></span>
                            <span className="shrink-0 text-xs uppercase tracking-wide text-muted-foreground">{h.kind === 'category' ? t('CATEGORIES') : h.kind === 'page' ? t('PAGES') : t('PRODUCTS')}</span>
                        </Link>
                    ))}
                    {seeAll && (
