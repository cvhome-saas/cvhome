...
/** Category listing: facets (ruled rail on desktop, drawer on mobile), sort, result count, ruled grid, page x of y. */
export function Listing({storeContext, category, initial, initialQuery, facets, grid}: {
    ...
    const facetPanel = hasFacets && (
        <div className="flex flex-col gap-6">
            {listing.facets.manufacturers.length > 0 && (
                <fieldset>
                    ...
                    <RadioGroup value={query.manufacturerId?.toString() ?? 'all'} onValueChange={v => listing.setManufacturer(v === 'all' ? undefined : Number(v))} className="gap-2.5">
                        <div className="flex items-center gap-2.5"><RadioGroupItem value="all" id="m-all"/><Label htmlFor="m-all" className="font-normal">{t('ALL')}</Label></div>
            ...
            {listing.hasActiveFilters && <Button variant="outline" size="sm" onClick={listing.clearFilters} className="self-start">{t('CLEAR_FILTERS')}</Button>}
        </div>
    );
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:gap-10">
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-4">
                <div className="flex flex-wrap items-center justify-between gap-3 border-b pb-3">
                    <p className="text-sm tabular-nums text-muted-foreground" aria-live="polite">{t('RESULTS_COUNT', {count: total})}</p>
                        ...
                        {hasFacets && (
                            <Drawer>
                                ...
                                <DrawerContent side="start" className="w-[88vw] max-w-sm overflow-y-auto">
                                    <DrawerHeader className="border-b"><DrawerTitle className="display text-2xl">{t('FILTERS')}</DrawerTitle><DrawerDescription className="sr-only">{t('FILTERS')}</DrawerDescription></DrawerHeader>
                ...
                {status === 'error' && (
                    <ErrorBlock title={t('LISTING_ERROR')} className="border" action={<Button variant="outline" onClick={listing.retry}>{tc('RETRY')}</Button>}>
                ...
                {totalPages > 1 && (
                    <nav className="mt-2 flex items-center justify-between gap-3 border-t pt-4" aria-label={t('PAGE_OF', {page: page + 1, total: totalPages})}>
                        ...
                        <span className="display text-sm tabular-nums text-muted-foreground">{t('PAGE_OF', {page: page + 1, total: totalPages})}</span>
