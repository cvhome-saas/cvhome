        ...
        <section aria-roledescription={slides.length > 1 ? 'carousel' : undefined} aria-label={storeName} className="cover relative">
            {slides.length > 0 ? (
                    ...
                    <Swiper key={dir} dir={dir} modules={[Pagination, Autoplay, A11y]} loop={slides.length > 1}
                        ...
                        {slides.map((s, i) => (
                            <SwiperSlide key={`${s.url}-${i}`} aria-label={t('HERO_SLIDE', {index: i + 1, total: slides.length})}>
                                <Image src={s.url} alt="" fill priority={i === 0} sizes="(max-width: 1344px) 100vw, 1344px" className="object-cover"/>
                            </SwiperSlide>
                        ))}
                    </Swiper>
                </div>
                ...
                <div className={cn('relative border', stage)}>
                    <Image src={banner.path} alt="" fill priority sizes="(max-width: 1344px) 100vw, 1344px" className="object-cover"/>
                </div>
            ) : null}
