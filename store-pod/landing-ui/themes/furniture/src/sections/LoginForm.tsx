        ...
        <div className="flex flex-col gap-6">
            ...
            <form method="post" action={data.action} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                    <Label htmlFor="username" className="sign text-[0.625rem] text-muted-foreground">{t('USERNAME')}</Label>
                    <Input id="username" name="username" autoComplete="username" required autoFocus/>
                </div>
                <div className="flex flex-col gap-2">
                    <Label htmlFor="password" className="sign text-[0.625rem] text-muted-foreground">{t('PASSWORD')}</Label>
                    <Input id="password" name="password" type="password" autoComplete="current-password" required/>
                </div>
                <Button type="submit" size="lg" className="sign px-8 text-[0.6875rem]">{t('SUBMIT')}</Button>
            </form>
