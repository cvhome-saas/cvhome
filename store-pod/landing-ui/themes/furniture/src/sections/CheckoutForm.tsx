...
/** The delivery instruction sheet: every field labelled the way a sign is, behaviour entirely `useCheckoutForm`. */
export function CheckoutForm({storeContext, requireLogin}: { storeContext: StoreContext; requireLogin: boolean }) {
    ...
    return (
        <>
            ...
            <form onSubmit={f.handleSubmit(f.onSubmit)} noValidate className="flex flex-col gap-7">
                <h2 className="sign-lg rule-brass border-b pb-3 text-lg">{t('FORM_TITLE')}</h2>
                ...
                <Button type="submit" size="lg" className="sign self-start px-8 text-[0.6875rem]">{t('PLACE_ORDER')}</Button>
            </form>
        </>
    );
}
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                <AlertDialogHeader className="items-center text-center">
                    ...
                    <AlertDialogTitle className="sign-lg text-lg">{t('ORDER_PLACED_SUCCESSFULLY')}</AlertDialogTitle>
...
function AgreementDialog({box, open, onOpenChange, setIsAgree}: { box: AgreementText | undefined; open: boolean; onOpenChange: (o: boolean) => void; setIsAgree: (a: boolean) => void }) {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                <AlertDialogHeader>
                    <AlertDialogTitle className="sign-lg text-lg">{t('TERMS_AND_CONDITIONS')}</AlertDialogTitle>
        ...
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent>
                <AlertDialogHeader className="items-center text-center">
                    ...
                    <AlertDialogTitle className="sign-lg text-lg">{t('LOGIN_REQUIRED_TITLE')}</AlertDialogTitle>
