...
export function SearchBox({storeContext, capabilities, className}: { storeContext: StoreContext; capabilities: SearchCapabilities; className?: string }) {
    ...
    return (
        <div className={cn('relative', className)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setFocused(false); }}>
            <form role="search" onSubmit={submit}>
                <label className="relative block">
                    ...
                    <Input type="search" role="combobox" aria-expanded={open} aria-controls={combobox.listboxId} aria-autocomplete="list" {...combobox.inputProps} value={query}
                           ...
                           className="rule-brass w-full rounded-control ps-8 pe-8"/>
                    {query && (
            ...
            {open && (
                <div id={combobox.listboxId} role="listbox" className="rule-brass absolute end-0 top-full z-50 mt-1.5 w-72 rounded-overlay border bg-popover p-1.5 text-popover-foreground shadow-overlay">
                    {!capabilities.text && <p className="sign rule-brass mb-1 border-b px-2 pb-2 pt-1 text-[0.5625rem] text-muted-foreground">{t('SUGGESTIONS_ONLY')}</p>}
                    ...
                    {hits.map((h, i) => (
                        <Link key={h.id} prefetch={false} href={h.href} {...combobox.optionProps(i)} onClick={clear}
                              className="flex items-center justify-between gap-3 rounded-control px-2 py-2 text-sm hover:bg-secondary focus:bg-secondary data-[active=true]:bg-secondary">
                            <span className="truncate">{h.title}</span>
                            <span className="sign shrink-0 text-[0.5625rem] text-muted-foreground">{h.kind === 'category' ? t('CATEGORIES') : h.kind === 'page' ? t('PAGES') : t('PRODUCTS')}</span>
                        </Link>
                    ))}
                    {seeAll && (
