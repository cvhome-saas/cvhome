...
export function SearchBox({storeContext, capabilities, className}: { storeContext: StoreContext; capabilities: SearchCapabilities; className?: string }) {
    ...
    return (
        <div className={cn('relative', className)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setFocused(false); }}>
            ...
            {open && (
                <div id="search-suggestions" role="listbox" className="rule-brass absolute end-0 top-full z-50 mt-1.5 w-72 rounded-overlay border bg-popover p-1.5 text-popover-foreground shadow-overlay">
                    ...
                    {result?.hits.map(h => (
                        <Link key={h.id} prefetch={false} href={h.href} role="option" onClick={clear}
                              className="flex items-center justify-between gap-3 rounded-control px-2 py-2 text-sm hover:bg-secondary focus:bg-secondary">
