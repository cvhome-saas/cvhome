...
/** Main image + thumbnail strip. Resets to the first image when the (variant-aware) image set changes. */
                    ...
                    {images.map((img, i) => (
                        <li key={img.id ?? img.imageUrl}>
                            <button type="button" onClick={() => setIndex(i)} aria-current={i === index}
                                    ...
                                    className={cn('relative size-16 overflow-hidden rounded-image border-2 bg-muted', i === index ? 'border-primary' : 'border-transparent hover:border-border')}>
