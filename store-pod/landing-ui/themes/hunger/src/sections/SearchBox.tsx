...
export function SearchBox({storeContext, capabilities, className}: { storeContext: StoreContext; capabilities: SearchCapabilities; className?: string }) {
    ...
    return (
        <div className={cn('relative', className)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setFocused(false); }}>
            <form role="search" onSubmit={submit}>
                <label className="relative block">
                    ...
                    <Input type="search" role="combobox" aria-expanded={open} aria-controls={combobox.listboxId} aria-autocomplete="list" {...combobox.inputProps} value={query}
                           ...
                           onChange={e => { setQuery(e.target.value); setFocused(true); }} onFocus={() => setFocused(true)} onClick={() => setFocused(true)} className="w-56 border-foreground ps-8 pe-8"/>
                    {query && (
            ...
            {open && (
                <div id={combobox.listboxId} role="listbox" className="absolute end-0 top-full z-50 mt-1 w-72 border-2 border-foreground bg-popover p-1 text-popover-foreground shadow-overlay">
                    {!capabilities.text && <p className="press border-b border-border px-2 py-1 text-xs tracking-wide text-muted-foreground">{t('SUGGESTIONS_ONLY')}</p>}
                    ...
                    {hits.map((h, i) => (
                        <Link key={h.id} prefetch={false} href={h.href} {...combobox.optionProps(i)} onClick={clear}
                              className="flex items-center justify-between gap-2 px-2 py-1.5 text-sm hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground data-[active=true]:bg-primary data-[active=true]:text-primary-foreground">
                            <span className="truncate">{h.title}</span>
                            <span className="press shrink-0 text-xs tracking-wide opacity-70">{h.kind === 'category' ? t('CATEGORIES') : h.kind === 'page' ? t('PAGES') : t('PRODUCTS')}</span>
                        </Link>
                    ))}
                    {seeAll && (
