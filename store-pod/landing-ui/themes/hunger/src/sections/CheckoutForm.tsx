...
function Field({id, label, error, children, className}: FieldProps) {
    return (
        <div className={cn('flex flex-col gap-1.5', className)} data-invalid={!!error || undefined}>
            <Label htmlFor={id} className="press text-xs tracking-wide text-muted-foreground">{label}</Label>
            {children}
    ...
    return (
        <>
            ...
            <form onSubmit={f.handleSubmit(f.onSubmit)} noValidate className="flex flex-col gap-6">
                <h2 className="press border-b-2 border-foreground pb-1 text-2xl">{t('FORM_TITLE')}</h2>
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    <Field id="country" label={t('COUNTRY')} error={e.customer?.billing?.country?.message}>
                        <Select onValueChange={v => f.setValue('customer.billing.country', v, {shouldValidate: true})} defaultValue={defaultCheckoutValue.customer.billing.country || undefined}>
                            <SelectTrigger id="country" className="w-full border-foreground" {...inv(e.customer?.billing?.country?.message)}><SelectValue placeholder={t('SELECT_COUNTRY')}/></SelectTrigger>
                ...
                <Field id="paymentType" label={t('PAYMENT_TYPE')} error={e.paymentType?.message}>
                    <Select onValueChange={v => f.setValue('paymentType', v as never, {shouldValidate: true})} defaultValue={f.supportedPaymentTypes?.[0]}>
                        <SelectTrigger id="paymentType" className="w-full border-foreground" {...inv(e.paymentType?.message)}><SelectValue placeholder={t('SELECT_PAYMENT_TYPE')}/></SelectTrigger>
                        <SelectContent className="border-2 border-foreground">
                ...
                <button type="submit" className="fold plate h-12 justify-center self-start border-primary px-8 text-base">{t('PLACE_ORDER')}</button>
            </form>
        </>
    );
}
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="border-2 border-foreground">
                <AlertDialogHeader>
                    <AlertDialogTitle className="press text-2xl">{t('ORDER_PLACED_SUCCESSFULLY')}</AlertDialogTitle>
                    <AlertDialogDescription>{order ? `${t('ORDER_ID')}: ${order.id}` : t('NO_ORDER_DETAILED')}</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogAction className="fold plate h-11 w-full justify-center border-primary"
                                       onClick={() => { onOpenChange(false); router.push('/'); }}>{t('CONTINUE_SHOPPING')}</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
...
function AgreementDialog({box, open, onOpenChange, setIsAgree}: { box: AgreementText | undefined; open: boolean; onOpenChange: (o: boolean) => void; setIsAgree: (a: boolean) => void }) {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="border-2 border-foreground">
                <AlertDialogHeader>
                    <AlertDialogTitle className="press text-2xl">{t('TERMS_AND_CONDITIONS')}</AlertDialogTitle>
                ...
                <AlertDialogFooter>
                    <AlertDialogCancel className="fold h-11 justify-center" onClick={() => setIsAgree(false)}>{t('REJECT')}</AlertDialogCancel>
                    <AlertDialogAction className="fold plate h-11 justify-center border-primary" onClick={() => setIsAgree(true)}>{t('AGREE')}</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
...
function LoginRequiredDialog({open, onOpenChange, login}: { open: boolean; onOpenChange: (o: boolean) => void; login: () => void }) {
    ...
    return (
        <AlertDialog open={open} onOpenChange={onOpenChange}>
            <AlertDialogContent className="border-2 border-foreground">
                <AlertDialogHeader>
                    <AlertDialogTitle className="press text-2xl">{t('LOGIN_REQUIRED_TITLE')}</AlertDialogTitle>
                    <AlertDialogDescription>{t('LOGIN_REQUIRED_DESCRIPTION')}</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel className="fold h-11 justify-center">{t('CANCEL')}</AlertDialogCancel>
                    <AlertDialogAction className="fold plate h-11 justify-center border-primary" onClick={login}>{t('LOGIN')}</AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
        </AlertDialog>
    );
