    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h1 className="signage text-3xl">{t('ORDER_DETAILS')} <span className="text-muted-foreground">#{order.id}</span></h1>
                <span className="sticker sticker-outline text-sm">{status(order.orderStatus)}</span>
            </div>
            <div className="grid gap-8 lg:grid-cols-3">
                <div className="flex flex-col gap-8 lg:col-span-2">
                    <section className="overflow-hidden rounded-card border-2 bg-card">
                        <h2 className="signage px-4 pt-4 text-xl">{t('ORDER_ITEMS')}</h2>
                        <Table>
                            <TableHeader>
                                <TableRow>
                        ...
                        <div className="ms-auto flex max-w-xs flex-col gap-2 p-4">
                            ...
                            <Separator/>
                            <div className="flex items-end justify-between"><span className="signage text-lg">{t('TOTAL')}</span><span className="price text-2xl">{order.total?.value}</span></div>
                        </div>
                    </section>
                    <section className="rounded-card border-2 bg-card p-4">
                        <h2 className="signage mb-4 text-xl">{t('ORDER_HISTORY')}</h2>
                ...
                <div className="flex flex-col gap-6">
                    {[[t('SHIPPING_ADDRESS'), order.delivery], [t('BILLING_ADDRESS'), order.billing]].map(([title, a]) => {
                        ...
                        return (
                            <section key={title as string} className="rounded-card border-2 bg-card p-4">
                                <h2 className="signage mb-2 text-base">{title as string}</h2>
                                {addr ? (
