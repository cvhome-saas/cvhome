...
function Address({a, title}: { a: Customer['billing'] | undefined; title: string }) {
    return (
        <section className="rounded-card border-2 bg-card p-4">
            <h3 className="signage mb-2 text-base">{title}</h3>
            {a?.address ? (
    ...
    return (
        <Tabs defaultValue="profile" dir={dir}>
            ...
            <TabsContent value="profile">
                <dl className="grid gap-4 rounded-card border-2 bg-card p-4 sm:grid-cols-2">
            ...
            <TabsContent value="orders">
                {!orders?.content?.length ? <EmptyState kind="orders"/> : (
                    <div className="overflow-x-auto rounded-card border-2 bg-card">
                        <Table>
                            <TableHeader>
                                <TableRow>
