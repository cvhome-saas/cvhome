...
/** The floor: crates stacked at quantity. Columns follow the theme's layout config so every listing agrees. */
export function ProductGrid({products, storeContext, grid, className}: {
    ...
    return (
        <ul className={cn('grid gap-3 sm:gap-4', COLS[grid.base], SM[grid.sm], LG[grid.lg], XL[grid.xl], className)}>
            {products.map((p, i) => (
                <li key={p.id} className="flex"><ProductCard product={p} storeContext={storeContext} priority={i < 5} className="w-full"/></li>
            ))}
        </ul>
    );
}
