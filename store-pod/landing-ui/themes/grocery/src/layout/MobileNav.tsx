...
/** The aisle map: mobile navigation drawer (opens from the reading-start side). Closes on navigation. */
export function MobileNav({ctx, data}: { ctx: PageContext; data: LayoutData }) {
    ...
    const linkCls = 'signage block rounded-control px-3 py-2.5 text-lg hover:bg-muted';
    ...
    return (
        <Drawer open={open} onOpenChange={setOpen}>
            ...
            <DrawerContent side="start" className="w-[88vw] max-w-sm overflow-y-auto border-e-2 p-0">
                <DrawerHeader className="border-b-2">
                    <DrawerTitle className="signage text-xl">{tc('MENU')}</DrawerTitle>
                    <DrawerDescription className="sr-only">{t('MAIN_NAVIGATION')}</DrawerDescription>
                </DrawerHeader>
                <nav aria-label={t('MAIN_NAVIGATION')} className="flex flex-col gap-1 p-3">
                                    ...
                                    <AccordionContent className="ps-4">
                                        {children.map(child => (
                                            <Link key={child.code} prefetch={false} href={`/category/${child.description.friendlyUrl}`}
                                                  className="block rounded-control px-3 py-2 text-base font-medium hover:bg-muted">
                    ...
                    {pages.map(page => (
                        <Link key={page.code} prefetch={false} href={page.href} className={linkCls}>{page.name}</Link>
                    ))}
                    {extras.map(page => (
                        <Link key={page.href} prefetch={false} href={page.href} className={linkCls}>{page.label}</Link>
                    ))}
                </nav>
            </DrawerContent>
        </Drawer>
