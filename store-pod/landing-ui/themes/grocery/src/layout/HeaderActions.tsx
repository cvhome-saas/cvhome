...
/** The basket: a crate button whose count sticker stamps in on every change. */
            ...
            <Button variant="ghost" size="icon" className="relative" aria-label={t('CART_WITH_COUNT', {count})} onClick={() => setOpen(true)}>
                ...
                {count > 0 && (
                    <span key={count} aria-hidden
                          className="sticker stamp-in absolute -end-1 -top-1 min-w-5 justify-center bg-primary px-1 py-0 text-[0.7rem] leading-5 text-primary-foreground">
                        {count}
                    </span>
                )}
            </Button>
