...
/**
 * A search strip, honest about what the platform can do: hidden when nothing is searchable, suggestions-only
 * (categories/pages) today, full text search when a provider with `text` arrives. Results land on a sheet.
 */
    ...
    return (
        <div className={cn('relative', className)} onBlur={e => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setFocused(false); }}>
            <form role="search" onSubmit={submit}>
                <label className="strip relative block h-9 w-full px-0 [--tilt:0deg] md:w-64 focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-ring">
                    ...
                    <input type="search" role="combobox" aria-expanded={open} aria-controls={combobox.listboxId} aria-autocomplete="list" {...combobox.inputProps} value={query}
                           ...
                           onChange={e => { setQuery(e.target.value); setFocused(true); }} onFocus={() => setFocused(true)} onClick={() => setFocused(true)}
                           className="h-full w-full bg-transparent pe-8 ps-8 font-sans text-sm normal-case tracking-normal outline-none placeholder:text-muted-foreground [&::-webkit-search-cancel-button]:hidden"/>
                    {query && (
                        <button type="button" aria-label={t('LABEL')} onClick={clear} className="absolute end-1 top-1/2 flex size-7 -translate-y-1/2 items-center justify-center hover:bg-foreground hover:text-background">
            ...
            {open && (
                <div id={combobox.listboxId} role="listbox" className="sheet absolute end-0 top-full z-50 mt-2 w-72 p-1 [--tilt:0deg]">
                    {!capabilities.text && <p className="px-2 py-1 text-xs text-muted-foreground">{t('SUGGESTIONS_ONLY')}</p>}
                    {loading && !result && <p className="px-2 py-2 text-sm text-muted-foreground">{tc('LOADING')}</p>}
                    ...
                    {hits.map((h, i) => (
                        <Link key={h.id} prefetch={false} href={h.href} {...combobox.optionProps(i)} onClick={clear}
                              className="flex items-center justify-between gap-2 px-2 py-1.5 text-sm hover:bg-foreground hover:text-background focus:bg-foreground focus:text-background data-[active=true]:bg-foreground data-[active=true]:text-background">
                            <span className="truncate">{h.title}</span>
                            <span className="shrink-0 font-display text-xs uppercase tracking-wide opacity-70">{h.kind === 'category' ? t('CATEGORIES') : h.kind === 'page' ? t('PAGES') : t('PRODUCTS')}</span>
                        </Link>
                    ))}
                    {seeAll && (
