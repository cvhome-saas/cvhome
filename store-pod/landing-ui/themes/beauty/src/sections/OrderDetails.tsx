    ...
    return (
        <div className="flex flex-col gap-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h1 className="q font-display text-3xl font-bold uppercase tracking-tight">{t('ORDER_DETAILS')} <span className="text-muted-foreground">#{order.id}</span></h1>
                <Badge variant="outline" className="q rounded-none border-foreground font-mono text-xs uppercase tracking-wide">{status(order.orderStatus)}</Badge>
            </div>
            <div className="grid gap-8 lg:grid-cols-3">
                <div className="flex flex-col gap-8 lg:col-span-2">
                    <section className="plate">
                        <h2 className="q border-b border-foreground px-4 py-2 font-display text-base font-semibold uppercase tracking-wide">{t('ORDER_ITEMS')}</h2>
                        <Table>
                            <TableHeader>
                                <TableRow>
                        ...
                        <div className="ms-auto flex max-w-xs flex-col gap-2 p-4">
                            ...
                            <div className="hazard-soft h-1.5" aria-hidden/>
                    ...
                    <section className="plate p-4">
                        <h2 className="q mb-4 font-display text-base font-semibold uppercase tracking-wide">{t('ORDER_HISTORY')}</h2>
                        <ol className="flex flex-col gap-4 border-s border-foreground ps-5">
                            {history.map((h, i) => (
                                <li key={i} className="relative">
                                    <span className="absolute -start-[1.45rem] top-1.5 size-2.5 bg-primary" aria-hidden/>
                ...
                <div className="flex flex-col gap-6">
                    {[[t('SHIPPING_ADDRESS'), order.delivery], [t('BILLING_ADDRESS'), order.billing]].map(([title, a]) => {
                        ...
                        return (
                            <section key={title as string} className="plate p-4">
                                <h2 className="q mb-2 font-display text-sm font-semibold uppercase tracking-wide">{title as string}</h2>
                                {addr ? (
