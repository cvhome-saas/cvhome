...
function Address({a, title}: { a: Customer['billing'] | undefined; title: string }) {
    return (
        <section className="plate p-4">
            <h3 className="q mb-2 font-display text-sm font-semibold uppercase tracking-wide">{title}</h3>
            {a?.address ? (
...
export function CustomerTabs({storeContext}: { storeContext: StoreContext }) {
    ...
    return (
        <Tabs defaultValue="profile" dir={dir}>
            <TabsList className="mb-6 grid h-auto w-full grid-cols-3 rounded-none border border-foreground bg-background p-0">
                <TabsTrigger className="q rounded-none border-e border-foreground font-display text-sm font-semibold uppercase tracking-wide last:border-e-0 data-[state=active]:bg-foreground data-[state=active]:text-background data-[state=active]:shadow-none" value="profile">{t('PROFILE')}</TabsTrigger>
                <TabsTrigger className="q rounded-none border-e border-foreground font-display text-sm font-semibold uppercase tracking-wide last:border-e-0 data-[state=active]:bg-foreground data-[state=active]:text-background data-[state=active]:shadow-none" value="addresses">{t('ADDRESSES')}</TabsTrigger>
                <TabsTrigger className="q rounded-none border-e border-foreground font-display text-sm font-semibold uppercase tracking-wide last:border-e-0 data-[state=active]:bg-foreground data-[state=active]:text-background data-[state=active]:shadow-none" value="orders">{t('ORDERS')}</TabsTrigger>
            </TabsList>
            <TabsContent value="profile">
                <dl className="grid gap-4 plate p-4 sm:grid-cols-2">
                            ...
                            <TableBody>
                                {orders.content.map(o => (
                                    <TableRow key={o.id}>
                                        ...
                                        <TableCell><Badge variant="outline" className="q rounded-none border-foreground font-mono text-[0.65rem] uppercase tracking-wide">{t.has(o.orderStatus) ? t(o.orderStatus) : o.orderStatus}</Badge></TableCell>
