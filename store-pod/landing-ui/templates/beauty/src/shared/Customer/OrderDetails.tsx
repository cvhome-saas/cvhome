...
export const OrderDetails = ({ storeContext, orderId }: { storeContext: StoreContext, orderId: number }) => {
    ...
    return (
        <div className={cn("max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8 bg-background", isRtlLayout ? "text-right" : "text-left")}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between mb-12 mt-10 gap-6">
                ...
                <Badge className="text-xs uppercase tracking-widest px-8 py-3 bg-primary text-white border-none rounded-full shadow-lg shadow-primary/20">
                ...
                <div className="lg:col-span-2 space-y-12">
                    <Card className="border-2 border-primary/10 shadow-none rounded-[2rem] overflow-hidden">
                        <CardHeader className="bg-primary/5">
                            <CardTitle className={cn("text-xl", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_ITEMS')}</CardTitle>
                        </CardHeader>
                        <CardContent className="p-0">
                            <Table>
                                <TableHeader className="bg-muted/20">
                                    <TableRow className="border-none">
                                        <TableHead className={cn("py-6 px-8", isRtlLayout ? "text-right" : "text-left")}>{t('PRODUCT')}</TableHead>
                                        <TableHead className="text-center py-6">{t('QTY')}</TableHead>
                                        <TableHead className={cn("py-6", isRtlLayout ? "text-left" : "text-right")}>{t('PRICE')}</TableHead>
                                        <TableHead className={cn("py-6 px-8", isRtlLayout ? "text-left" : "text-right")}>{t('TOTAL')}</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {order&&order.products && order.products.map((product) => (
                                        <TableRow key={product.id} className="border-primary/5">
                                            <TableCell className={cn("font-bold py-6 px-8", isRtlLayout ? "text-right" : "text-left")}>
                                                <span className="text-foreground/80">{product.productName}</span>
                                            </TableCell>
                                            <TableCell className="text-center font-medium text-muted-foreground">{product.orderedQuantity}</TableCell>
                                            <TableCell className={cn("font-medium", isRtlLayout ? "text-left" : "text-right")}>{product.price}</TableCell>
                                            <TableCell className={cn("font-bold text-primary px-8", isRtlLayout ? "text-left" : "text-right")}>{product.subTotal}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
                            <div className={cn("p-8 bg-primary/5 space-y-4", isRtlLayout ? "text-left" : "text-right")}>
                                ...
                                <div className={cn("flex justify-between max-w-xs text-2xl font-black text-primary", isRtlLayout ? "me-auto" : "ms-auto")}>
                                    <span>{t('TOTAL')}</span>
                    ...
                    <Card className="border-2 border-primary/10 shadow-none rounded-[2rem] overflow-hidden">
                        <CardHeader className="bg-primary/5">
                            <CardTitle className={cn("text-xl", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_HISTORY')}</CardTitle>
                        </CardHeader>
                        <CardContent className="p-8 space-y-10">
                            {history&&history.map((h, i) => (
                                <div key={i} className={cn("flex gap-8 pb-10 relative last:pb-0", isRtlLayout ? "border-e-2 border-primary/10 pe-10" : "border-s-2 border-primary/10 ps-10")}>
                                    <div className={cn("absolute top-0 size-6 rounded-full bg-background border-4 border-primary shadow-sm shadow-primary/20", isRtlLayout ? "-end-[13px]" : "-start-[13px]")} />
                                    <div className="flex-grow">
                                        ...
                                        {h.comments && <p className="text-sm text-muted-foreground leading-relaxed bg-muted/30 p-4 rounded-2xl italic">{h.comments}</p>}
                                    </div>
                                </div>
                            ))}
                        </CardContent>
                ...
                <div className="space-y-8">
                        <Card className="border-2 border-primary/10 shadow-none rounded-[2rem]">
                            <CardHeader className="pb-4">
                                <CardTitle className={cn("text-lg font-bold uppercase tracking-widest text-primary/60", isRtlLayout ? "text-right" : "text-left")}>{t('SHIPPING_ADDRESS')}</CardTitle>
                            </CardHeader>
                        ...
                        <Card className="border-2 border-primary/10 shadow-none rounded-[2rem]">
                            <CardHeader className="pb-4">
                                <CardTitle className={cn("text-lg font-bold uppercase tracking-widest text-primary/60", isRtlLayout ? "text-right" : "text-left")}>{t('BILLING_ADDRESS')}</CardTitle>
                            </CardHeader>
                            {order &&<CardContent className="space-y-2">
