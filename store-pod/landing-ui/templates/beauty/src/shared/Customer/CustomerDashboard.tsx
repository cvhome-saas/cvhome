        ...
        <div className={cn("max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8", isRtlLayout ? "text-right" : "text-left")}>
            ...
            <Tabs defaultValue="profile" className="w-full" dir={isRtlLayout ? "rtl" : "ltr"}>
                <TabsList className="grid w-full grid-cols-3 mb-12 bg-primary/5 p-1 rounded-2xl">
                    <TabsTrigger value="profile" className="rounded-xl py-3 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all">{t('PROFILE')}</TabsTrigger>
                    <TabsTrigger value="addresses" className="rounded-xl py-3 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all">{t('ADDRESSES')}</TabsTrigger>
                    <TabsTrigger value="orders" className="rounded-xl py-3 data-[state=active]:bg-background data-[state=active]:shadow-sm transition-all">{t('ORDERS')}</TabsTrigger>
                </TabsList>

                <TabsContent value="profile" className="space-y-6">
                    <Card className="border-2 border-primary/10 shadow-none rounded-[2rem] overflow-hidden">
                        <CardHeader className="bg-primary/5 pb-8">
                            <CardTitle className={cn("text-2xl", isRtlLayout ? "text-right" : "text-left")}>{t('PERSONAL_INFO')}</CardTitle>
                        </CardHeader>
                        <CardContent className="pt-8 space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-primary/60">{t('FIRST_NAME')}</Label>
                                    <p className="text-lg font-medium text-foreground/80">{customer?.firstNames}</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-primary/60">{t('LAST_NAME')}</Label>
                                    <p className="text-lg font-medium text-foreground/80">{customer?.lastName}</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-primary/60">{t('EMAIL')}</Label>
                                    <p className="text-lg font-medium text-foreground/80">{customer?.emailAddress}</p>
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-primary/60">{t('PHONE')}</Label>
                                    <p className="text-lg font-medium text-foreground/80">{customer?.billing?.phone}</p>
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                    <Label className="text-xs font-bold uppercase tracking-widest text-primary/60">{t('USERNAME')}</Label>
                ...
                <TabsContent value="addresses" className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Card className="border-2 border-primary/10 shadow-none rounded-[2rem]">
                            <CardHeader>
                                <CardTitle className={cn("text-xl", isRtlLayout ? "text-right" : "text-left")}>{t('BILLING_ADDRESS')}</CardTitle>
                            </CardHeader>
                        ...
                        <Card className="border-2 border-primary/10 shadow-none rounded-[2rem]">
                            <CardHeader>
                                <CardTitle className={cn("text-xl", isRtlLayout ? "text-right" : "text-left")}>{t('SHIPPING_ADDRESS')}</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-3">
                                    ...
                                    <Table>
                                        <TableHeader className="bg-muted/30">
                                            <TableRow className="border-none">
                                                <TableHead className={cn("py-6 px-8", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_ID')}</TableHead>
                                                <TableHead className={cn("py-6", isRtlLayout ? "text-right" : "text-left")}>{t('DATE')}</TableHead>
                                                <TableHead className={cn("py-6", isRtlLayout ? "text-right" : "text-left")}>{t('TOTAL')}</TableHead>
                                                <TableHead className={cn("py-6", isRtlLayout ? "text-right" : "text-left")}>{t('STATUS')}</TableHead>
                                                <TableHead className={cn("py-6 px-8", isRtlLayout ? "text-left" : "text-right")}>{t('ACTIONS')}</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {orders.content.map((order) => (
                                                <TableRow key={order.id} className="border-primary/5 hover:bg-primary/5 transition-colors">
                                                    ...
                                                    <TableCell className="text-foreground/70">{new Date(order.datePurchased).toLocaleDateString()}</TableCell>
                                                    <TableCell className="font-bold text-primary">{order?.total?.value}</TableCell>
                                                    <TableCell>
                                                    ...
                                                    <TableCell className={cn("py-6 px-8", isRtlLayout ? "text-left" : "text-right")}>
                                                        <Button variant="ghost" asChild size="sm" className="rounded-full hover:bg-primary hover:text-white transition-all px-6">
