        ...
        <div className={cn("max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8", isRtlLayout ? "text-right" : "text-left")}>
            ...
            <Tabs defaultValue="profile" className="w-full" dir={isRtlLayout ? "rtl" : "ltr"}>
                <TabsList className="grid w-full grid-cols-3 mb-8 bg-secondary/20">
                    <TabsTrigger value="profile" className="uppercase tracking-widest text-xs">{t('PROFILE')}</TabsTrigger>
                    <TabsTrigger value="addresses" className="uppercase tracking-widest text-xs">{t('ADDRESSES')}</TabsTrigger>
                    <TabsTrigger value="orders" className="uppercase tracking-widest text-xs">{t('ORDERS')}</TabsTrigger>
                </TabsList>

                <TabsContent value="profile">
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('PERSONAL_INFO')}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-1">
                                    <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">{t('FIRST_NAME')}</Label>
                                    <p className="font-medium border-b border-border pb-2">{customer?.firstNames}</p>
                                </div>
                                <div className="space-y-1">
                                    <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">{t('LAST_NAME')}</Label>
                                ...
                                <div className="space-y-1">
                                    <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">{t('PHONE')}</Label>
                                    <p className="font-medium border-b border-border pb-2">{customer?.billing?.phone}</p>
                                </div>
                                <div className="space-y-1">
                                    <Label className="text-[10px] uppercase tracking-widest text-muted-foreground">{t('USERNAME')}</Label>
                ...
                <TabsContent value="addresses" className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <Card className="border-none shadow-none bg-secondary/5">
                            <CardHeader>
                                <CardTitle className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('BILLING_ADDRESS')}</CardTitle>
                            </CardHeader>
                        ...
                        <Card className="border-none shadow-none bg-secondary/5">
                            <CardHeader>
                                <CardTitle className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('SHIPPING_ADDRESS')}</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-2 text-sm">
                ...
                <TabsContent value="orders">
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('ORDERS')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                                    ...
                                    <Table>
                                        <TableHeader>
                                            <TableRow className="border-border">
                                                <TableHead className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_ID')}</TableHead>
                                                <TableHead className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-right" : "text-left")}>{t('DATE')}</TableHead>
                                                <TableHead className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-right" : "text-left")}>{t('TOTAL')}</TableHead>
                                                <TableHead className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-right" : "text-left")}>{t('STATUS')}</TableHead>
                                                <TableHead className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-left" : "text-right")}>{t('ACTIONS')}</TableHead>
                                            </TableRow>
                                        </TableHeader>
                                        <TableBody>
                                            {orders.content.map((order) => (
                                                <TableRow key={order.id} className="border-border/50">
                                                    ...
                                                    <TableCell className="text-muted-foreground">{new Date(order.datePurchased).toLocaleDateString()}</TableCell>
                                                    <TableCell className="font-semibold">{order?.total?.value}</TableCell>
                                                    <TableCell>
                                                        <span className="inline-block px-2 py-1 bg-primary/10 text-primary text-[10px] uppercase tracking-widest rounded-full font-bold">
                                                            {t(order?.orderStatus??'UNKNOWN')}
                                                        </span>
                                                    </TableCell>
                                                    <TableCell className={cn(isRtlLayout ? "text-left" : "text-right")}>
                                                        <Button variant="outline" asChild size="sm" className="rounded-none border-primary/20 hover:border-primary hover:bg-primary/5 uppercase tracking-widest text-[10px]">
