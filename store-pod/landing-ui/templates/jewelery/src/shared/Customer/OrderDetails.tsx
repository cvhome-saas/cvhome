...
export const OrderDetails = ({storeContext, orderId}: { storeContext: StoreContext, orderId: number }) => {
    ...
    return (
        <div
            className={cn("max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 bg-background", isRtlLayout ? "text-right" : "text-left")}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between mb-12 mt-8 gap-4">
                ...
                <Badge variant="outline"
                       className="text-[10px] uppercase tracking-widest px-6 py-2 border-primary text-primary bg-primary/5 rounded-none">{t(order?.orderStatus ?? 'UNKNOWN')}</Badge>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-12">
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle
                                className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_ITEMS')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                    <TableRow className="border-border">
                                        <TableHead
                                            className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-right" : "text-left")}>{t('PRODUCT')}</TableHead>
                                        <TableHead
                                            className="text-center uppercase tracking-widest text-[10px]">{t('QTY')}</TableHead>
                                        <TableHead
                                            className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-left" : "text-right")}>{t('PRICE')}</TableHead>
                                        <TableHead
                                            className={cn("uppercase tracking-widest text-[10px]", isRtlLayout ? "text-left" : "text-right")}>{t('TOTAL')}</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {order && order.products && order.products.map((product) => (
                                        <TableRow key={product.id} className="border-border/50">
                                            <TableCell
                                                className={cn("font-medium", isRtlLayout ? "text-right" : "text-left")}>
                                                <p className="uppercase tracking-wider text-xs">{product.productName}</p>
                                            </TableCell>
                                            <TableCell
                                                className="text-center text-muted-foreground">{product.orderedQuantity}</TableCell>
                                            <TableCell
                                                className={cn("text-muted-foreground", isRtlLayout ? "text-left" : "text-right")}>{product.price}</TableCell>
                                            <TableCell
                                                className={cn("font-semibold", isRtlLayout ? "text-left" : "text-right")}>{product.subTotal}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>
                            <div className={cn("mt-12 space-y-3", isRtlLayout ? "text-left" : "text-right")}>
                                <div
                                    ...
                                    <span
                                        className="text-muted-foreground uppercase tracking-widest text-[10px]">{t('SUB_TOTAL')}</span>
                    ...
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle
                                className={cn("font-serif tracking-wide", isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_HISTORY')}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-8 pt-4">
                            {history && history.map((h, i) => (
                                <div key={i}
                                     className={cn("flex gap-6 border-border pb-8 relative last:pb-0", isRtlLayout ? "border-e border-dashed pe-8" : "border-s border-dashed ps-8")}>
                                    <div
                                        className={cn("absolute top-0 size-3 border border-primary bg-background rotate-45", isRtlLayout ? "-end-[7px]" : "-start-[7px]")}/>
                                    <div>
                                        ...
                                        {h.comments &&
                                            <p className="mt-3 text-sm italic font-serif text-muted-foreground">{h.comments}</p>}
                                    </div>
                                </div>
                            ))}
                        </CardContent>
                ...
                <div className="space-y-12">
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle
                                className={cn("font-serif tracking-wide text-sm", isRtlLayout ? "text-right" : "text-left")}>{t('SHIPPING_ADDRESS')}</CardTitle>
                        </CardHeader>
                    ...
                    <Card className="border-none shadow-none bg-secondary/5">
                        <CardHeader>
                            <CardTitle
                                className={cn("font-serif tracking-wide text-sm", isRtlLayout ? "text-right" : "text-left")}>{t('BILLING_ADDRESS')}</CardTitle>
                        </CardHeader>
                        {order && <CardContent className="space-y-2 text-sm">
