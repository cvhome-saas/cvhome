        ...
        <div className={cn("max-w-7xl mx-auto py-10 px-4 sm:px-6 lg:px-8 bg-background", isRtlLayout ? "text-right" : "text-left")}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-8">
                    <Card>
                        <CardHeader>
                            <CardTitle className={cn(isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_ITEMS')}</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead className={cn(isRtlLayout ? "text-right" : "text-left")}>{t('PRODUCT')}</TableHead>
                                        <TableHead className="text-center">{t('QTY')}</TableHead>
                                        <TableHead className={cn(isRtlLayout ? "text-left" : "text-right")}>{t('PRICE')}</TableHead>
                                        <TableHead className={cn(isRtlLayout ? "text-left" : "text-right")}>{t('TOTAL')}</TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                    {order&&order.products && order.products.map((product) => (
                                        <TableRow key={product.id}>
                                            <TableCell className={cn("font-medium", isRtlLayout ? "text-right" : "text-left")}>{product.productName}</TableCell>
                                            <TableCell className="text-center">{product.orderedQuantity}</TableCell>
                                            <TableCell className={cn(isRtlLayout ? "text-left" : "text-right")}>{product.price}</TableCell>
                            ...
                            <div className={cn("mt-8 space-y-2", isRtlLayout ? "text-left" : "text-right")}>
                                ...
                                <Separator />
                                <div className={cn("flex justify-between max-w-xs text-xl font-bold", isRtlLayout ? "me-auto" : "ms-auto")}>
                                    <span>{t('TOTAL')}</span>
                                    <span>{order?.total?.value}</span>
                                </div>
                            </div>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className={cn(isRtlLayout ? "text-right" : "text-left")}>{t('ORDER_HISTORY')}</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-4">
                            {history&&history.map((h, i) => (
                                <div key={i} className={cn("flex gap-4 border-border pb-6 relative last:pb-0", isRtlLayout ? "border-e-2 pe-6" : "border-s-2 ps-6")}>
                                    <div className={cn("absolute top-0 size-4 rounded-full bg-primary", isRtlLayout ? "-end-[9px]" : "-start-[9px]")} />
                                    <div>
