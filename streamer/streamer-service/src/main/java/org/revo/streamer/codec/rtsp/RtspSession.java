package org.revo.streamer.codec.rtsp;

import gov.nist.javax.sdp.SessionDescriptionImpl;
import io.netty.handler.codec.http.DefaultFullHttpRequest;
import lombok.Getter;
import org.revo.streamer.codec.commons.rtp.d.InterLeavedRTPSession;
import org.revo.streamer.codec.commons.rtp.d.MediaStream;
import org.revo.streamer.codec.commons.utils.URLObject;
import org.revo.streamer.codec.sdp.SdpUtil;
...
public class RtspSession {
    ...
    public Transport setup(String url, Transport transport) throws TransportNotSupportedException {
        if (!Transport.RTP_AVP_TCP.equals(transport.getTranport())) {
            throw new TransportNotSupportedException(transport.getTranport());
        }

        if (!Transport.UNICAST.equals(transport.getUnicast())) {
...
//        /aslive/live/test                        rtsp://127.0.0.1:8085/aslive/live/test?l=10/streamid=0
        ...
        for (MediaDescription dm : getMediaDescriptions(sd)) {
            try {
                if (uri.endsWith(getControlUri(dm))) {
