...
@Component({
  ...
  template: `
    <app-panel [title]="t('content.snippets.title')" [subtitle]="t('content.snippets.subtitle')" *transloco="let t">
      <ul class="snippet-list">
        @for (code of codes(); track code) {
          <li [class.open]="openCode() === code">
            <button class="snippet-row" type="button" (click)="toggle(code)" [attr.aria-expanded]="openCode() === code">
              <span class="snippet-copy">
                <strong dir="ltr">{{ code }}</strong>
                <small>{{ describe(code) }}</small>
              </span>
              @if (byCode().get(code); as snippet) {
                <app-badge [tone]="snippet.visible ? 'green' : 'slate'" shape="square">{{ snippet.visible ? t('content.snippets.live') : t('content.snippets.hidden') }}</app-badge>
              } @else {
                <app-badge tone="amber" shape="square">{{ t('content.snippets.notSet') }}</app-badge>
              }
              <app-icon [name]="openCode() === code ? 'chevronDown' : 'chevronRight'" [size]="14" [flip]="openCode() !== code" />
            </button>
            @if (openCode() === code && draft(); as d) {
              <div class="snippet-editor">
                <div class="editor-head">
                  <app-locale-switcher [languages]="locales()" [(active)]="language" [filled]="filled()" display="label" [label]="t('content.copy.writeIn')" />
                  <app-toggle [checked]="d.visible" (checkedChange)="setVisible($event)" [label]="t('content.snippets.visible')" [disabled]="!canManage()" />
                </div>
                <label class="field">
                  <span class="field-label">{{ t('content.snippets.titleField') }}</span>
                  <app-text-field [value]="d.translations[language()]?.title ?? ''" (valueChange)="setTitle($event)" [maxLength]="120" [disabled]="!canManage()" />
                </label>
                <div class="field">
                  <span class="field-label">{{ t('content.snippets.bodyField') }}</span>
                  <app-rich-text [value]="d.translations[language()]?.body ?? ''" (valueChange)="setBody($event)" [disabled]="!canManage()" [ariaLabel]="t('content.snippets.bodyField')" />
                </div>
                @if (canManage()) {
                  <div class="editor-actions">
                    <button class="primary-action" type="button" [disabled]="saving()" (click)="save()"><app-icon name="check" />{{ t('content.action.save') }}</button>
                    <button class="ghost-action" type="button" (click)="openCode.set(null)">{{ t('shared.actions.cancel') }}</button>
                  </div>
                }
              </div>
            }
...
export class SnippetsCard {
  ...
  protected readonly codes = computed(() => {
    const extra = [...this.byCode().keys()].filter((code) => !(KNOWN_CODES as readonly string[]).includes(code) && code !== 'LANDING_PAGE');
    return [...KNOWN_CODES, ...extra];
  });

  protected readonly filled = computed<ReadonlySet<string>>(() => {
    const d = this.draft();
    return new Set(d ? Object.entries(d.translations).filter(([, t]) => t.title.trim() || t.body.trim()).map(([code]) => code) : []);
  });
  ...
  private patch(change: Partial<{title: string; body: string}>): void {
    ...
    this.draft.update((d) =>
      d ? {...d, translations: {...d.translations, [lang]: {...(d.translations[lang] ?? {title: '', body: ''}), ...change}}} : d,
    );
  }
  ...
  protected save(): void {
    ...
    this.api.put(d.code, {id: existing?.id, code: d.code, visible: d.visible, translations}).subscribe({
      next: () => {
