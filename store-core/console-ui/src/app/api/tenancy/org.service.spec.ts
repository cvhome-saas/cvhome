...
describe('OrgService', () => {
  ...
  it('wraps the first administrator in a user envelope, and sends no plan', () => {
    service
      .create({firstName: 'Ada', lastName: 'Lovelace', emailAddress: 'ada@example.com', password: 'Passw0rd'})
      .subscribe();
    const request = http.expectOne(scoped(`${BASE}/create`));
    expect(request.request.method).toBe('POST');
    expect(request.request.body).toEqual({
      user: {firstName: 'Ada', lastName: 'Lovelace', emailAddress: 'ada@example.com', password: 'Passw0rd'},
    });
    request.flush({});
  });
