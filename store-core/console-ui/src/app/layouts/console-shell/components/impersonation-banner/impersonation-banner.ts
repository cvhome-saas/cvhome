 ...
 * It names the merchant and the time left, and carries the only control that
...
@Component({
  ...
  template: `
    @if (shell.impersonation(); as acting) {
      <aside class="impersonation-banner" role="status" *transloco="let t">
        <app-icon name="signIn" [flip]="true" />
        <p>
          <strong>{{ t('shell.impersonation.actingAs', {name: acting.actingAs}) }}</strong>
          <span class="detail">{{ remainingLabel() }}</span>
        </p>
