...
public class UserAccountController {
    ...
    @ConditionalOnApiStatus
    public Mono<ReadableUserList> list(
            @AuthenticationPrincipal Principal principal,
            @OrgStorePrincipalInfo UserOrgStoreIdentity identity,
            @RequestParam ManagerStoreId store) {
        UserOrgStoreIdentity impersonateIdentity =
                createImpersonateIdentity(identity, store, "list-users");
    ...
    //    @PreAuthorize("hasPermission(#store,'ManagerStoreId','STORE.USERS.LIST')")
    @ConditionalOnApiStatus
    public Mono<ReadableUser> findOne(
            @OrgStorePrincipalInfo UserOrgStoreIdentity identity, @RequestParam String userId) {
    ...
    @ConditionalOnApiStatus
    public Mono<ReadableUser> create(
            @OrgStorePrincipalInfo UserOrgStoreIdentity identity,
            @RequestParam ManagerStoreId store,
            @RequestBody PersistableUser user) {
    ...
    @ConditionalOnApiStatus
    public Mono<ReadableUser> update(
            @OrgStorePrincipalInfo UserOrgStoreIdentity identity,
            @RequestParam ManagerStoreId store,
            @RequestBody PersistableUser user) {
        UserOrgStoreIdentity impersonateIdentity =
                createImpersonateIdentity(identity, store, "update-user");
    ...
    private UserOrgStoreIdentity createImpersonateIdentity(
            UserOrgStoreIdentity identity, ManagerStoreId store, String action) {
        if (identity.isSuperAdmin()) {
            ManagerStoreDto s = internalStoreService.findStore(store);
            log.info(
                    "Impersonating user {} for action {} to org {} with store {}",
                    identity,
                    action,
                    s.orgId(),
                    s.id().id().toString());
            return new UserOrgStoreIdentity(s.orgId(), s.id().id().toString(), identity.roles());
        } else {
            return identity;
        }
    }
