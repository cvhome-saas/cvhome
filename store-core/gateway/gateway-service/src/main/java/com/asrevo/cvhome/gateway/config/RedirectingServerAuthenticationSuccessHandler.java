    ...
    public Mono<Void> onAuthenticationSuccess(WebFilterExchange webFilterExchange, Authentication authentication) {
        ...
        return exchange.getSession().flatMap(webSession -> {
            ...
            if (!StringUtils.hasText(stateFromCallback) && authentication instanceof OAuth2AuthenticationToken) {
                log.warn("State parameter not found directly in callback query params. This might affect redirectTo retrieval.");
            }


            if (StringUtils.hasText(stateFromCallback)) {
                String sessionKey = CapturingServerOAuth2AuthorizationRequestResolver.CAPTURED_PARAMETERS_SESSION_KEY_PREFIX + stateFromCallback;
                ...
                if (capturedParamsObject instanceof Map) {
                    ...
                    if (StringUtils.hasText(redirectTo)) {
                        if (isValidRedirect(redirectTo)) {
                            ...
                            log.warn("Invalid or disallowed 'redirectTo' parameter: '{}'. Falling back to default.", redirectTo);
                        }
                    } else {
                    ...
                    log.warn("Captured parameters in session (key: {}) were not of expected Map type: {}. Falling back to default.",
                            sessionKey, capturedParamsObject.getClass().getName());
                    webSession.getAttributes().remove(sessionKey); // Clean up mismatched type
                } else {
                    log.debug("No captured parameters found in session for key: {}. Falling back to default.", sessionKey);
                }
            } else {
                log.warn("State parameter was missing or empty in OAuth2 callback. Cannot retrieve 'redirectTo'. Falling back to default.");
            }
