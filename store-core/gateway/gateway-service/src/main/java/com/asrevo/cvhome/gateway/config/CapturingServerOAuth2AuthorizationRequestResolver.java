...
public class CapturingServerOAuth2AuthorizationRequestResolver implements ServerOAuth2AuthorizationRequestResolver {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CapturingServerOAuth2AuthorizationRequestResolver.class);
    ...
    public CapturingServerOAuth2AuthorizationRequestResolver(ReactiveClientRegistrationRepository clientRegistrationRepository) {
    ...
    private Mono<OAuth2AuthorizationRequest> captureParametersAndStoreInSession(ServerWebExchange exchange, OAuth2AuthorizationRequest authorizationRequest) {
        ...
        if (!capturedParamsMap.isEmpty()) {
            ...
            if (state == null || state.isEmpty()) {
                logger.warn("OAuth2AuthorizationRequest state is null or empty. Cannot reliably store captured parameters for this flow.");
                return Mono.just(authorizationRequest); // Proceed without storing if state is missing
            }
