...
`fargate-config.yml` — plus a `services` entry in `lcl.yml` and a route on its edge. For a pod service that is a
...
uaa → pod service → database. Bring the whole stack up with **`./extra/scripts/lcl start -d`** (infra + every
...
| Bring the local stack up or shut it down | `references/qa-testing.md` §1 (`lcl`, its flags, `SIGINT`) |
...
**Running & QA**
- `references/qa-testing.md` — **how QA is done here**: `lcl` and its flags, the seeded demo logins and
