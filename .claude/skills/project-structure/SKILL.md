...
| `store-pod/catalog` | **BE** | 8122 | **Product catalog**: products, categories (materialised-path tree), brands, product types, product groups / related items, images. Rewritten 2026-08 as a small readable service: per domain `entity` + `repositories/<X>Repository` + `services/<domain>/{XService, XServiceImpl, XMapper}` (plain static/bean mappers, no facade/populator layers). Responses carry **no price or quantity** (inventory owns those, by sku). APIs (`api/v1`, `api/v2`): `CategoryApi`, `ManufacturerApi`, `ProductTypeApi`, `ProductGroupApi`, `ProductRelationshipApi`, `ProductImageApi`, `ProductApi` (small writes), `ProductApiV2` (definition + storefront listing/PDP), `ExternalProductApi` (s2s by sku). Variants and product options/attributes are parked in `store-pod/catalog-deprecated` (unregistered). |
| `store-pod/inventory` | **BE** | 8126 | **Stock, pricing & reservations**, split out of catalog and keyed by **sku** (schema `inventory`; `product_id` is informational only, no FK to catalog). Deliberately tiny — one entity (`Inventory` + `InventoryPrice`), one service each for stock and reservations, no facade/mapper/populator layers. APIs: `InventoryApi` (sku-addressed upsert `PUT /private/inventory/{sku}`, `DELETE /private/inventory/by-product/{id}`), `ExternalInventoryApi` (public bulk `GET /availability?skus=` → `SkuInventory` with raw amounts), `ExternalProductReservationApi` (`STORE-POD.INVENTORY.RESERVE`). Consumers merge price/stock client-side (console-ui, landing-ui) or compose s2s (checkout's `ProductDetailsComposer`). One-off data migration: `inventory-service/init-sql/migrate-from-catalog.sql`. |
...
platform (`gateway.com`, `uaa.`, `console-ui.`), pod (`spg-507f1f77.gateway.com`, `merchant.`, …) and the demo
...
`fargate-config.yml` — plus a service block in `lcl.yml` and a route on its edge. For a pod service that is a
...
`.route(...)`**, because that array is negated to build console-ui's catch-all, which otherwise swallows the
path. Miss one of these and you get "no instances available", a gateway 503, or console-ui's HTML instead of
your API.
...
uaa → pod service → database. Install `@cvhome-saas/lcl` globally, then bring the whole stack up with
**`lcl start -d`** (infra + every
...
Use `lcl start -d` to bring the stack up, `lcl status` / `lcl urls` to see it, `lcl restart <svc>` /
`lcl stop <svc>` for one service (the rest keep running), `lcl why <svc>` and `lcl logs <svc> --errors` when
something is wrong, and `lcl stop` to tear that stack down. Several named stacks can run at once
(`--stack xxx`); when the configured ports are taken a stack shifts to the next free +1000 sequence
(`lcl ports`). The engine is the public `@cvhome-saas/lcl` package; this repository owns only `lcl.yml`.
...
1. **`-ui` suffix = Gradle-wrapped npm app.** `console-ui` (Angular) and `landing-ui` (Next.js) both apply the
...
- `references/new-service.md` — **step-by-step procedure + checklist for creating a new service**: the three
  shapes (backend like `catalog`/`tenancy`, UI like `console-ui`, both-in-one like `uaa`), module layout
