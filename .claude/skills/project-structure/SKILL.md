...
| `store-pod/catalog` | **BE** | 8122 | **Product catalog**: product CRUD, categories, images, attributes/options, product types, groups, manufacturers, relationships. Since the catalog/inventory split its responses carry **no price or quantity**. APIs: `ProductApi`, `CategoryApi`, `ExternalProductApi` (product data only). Variant machinery is parked in `store-pod/catalog-deprecated` (unregistered). |
