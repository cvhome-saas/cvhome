...
| Consumers | `uaa`, `gateway-service`, `control-plane/*`, `cua`, and every `-service` (for `autoconfigure`) | Nearly every `store-pod` module: catalog, checkout, merchant, payment, customer, reference, cua, `store-cms-commons` |
