...
public class KeycloakUserAccountServiceImpl implements UserAccountService {
	...
	@Override
	public ReadableUser createOrgUser(ManagerOrgId orgId, PersistableUser persistableUser) {
		try {
			UserRepresentation user = userRepresentationMapper.copyPersistableUser(persistableUser);
			Map<String, List<String>> attributes = Map.of(USER_TYPE_ATTR_KEY, List.of(KCUserType.ORG_USER.name()),
					ORG_ATTR_KEY, List.of(orgId.id().toString()));
	...
	private ReadableUser doCreateKCUser(PersistableUser persistableUser, UserRepresentation user, boolean temporary) {
		...
		UserPassword passwordRequestDto = new UserPassword(persistableUser.getPassword(),
				persistableUser.getRepeatPassword());
	...
	public ReadableUserList list(Principal principal, UserOrgStoreIdentity identity, ManagerStoreId store) {
		...
		List<UserRepresentation> allUsers = new ArrayList<>(
				usersResource.searchByAttributes(new ListUsersQuery(identity.org(), store).query()));

		if (isSupperAdmin(identity))
			allUsers.addFirst(usersResource.get(identity.org().id().toString()).toRepresentation());

		List<UserRepresentation> usersExceptMe = allUsers.stream()
	...
	private ReadableUser createManagedUser(ManagerOrgId orgId, ManagerStoreId managerStoreId,
			PersistableUser persistableUser) {
		try {
			UserRepresentation user = userRepresentationMapper.copyPersistableUser(persistableUser);
			Map<String, List<String>> attributes = Map.of(USER_TYPE_ATTR_KEY, List.of(KCUserType.MANAGED_USER.name()),
					ORG_ATTR_KEY, List.of(orgId.id().toString()), STORE_ATTR_KEY,
					List.of(managerStoreId.getId().toString()));
			user.setAttributes(attributes);
	...
	public void changePassword(ManagerOrgId managerOrgId, UserPassword request) {
		// @TODO re impl reset password to use orgId
		// doResetPassword(request, owner.id(), false);
	}

	private void checkAttrAndValidate(UserOrgStoreIdentity userOrgStoreInfo, ManagerStoreId store,
			UserRepresentation representation, Runnable runnable) {
	...
	private boolean attrMatch(UserRepresentation representation, UserOrgStoreIdentity userOrgStoreInfo,
			ManagerStoreId store) {
		if (isSupperAdmin(userOrgStoreInfo))
			return true;
		...
		if (!orgAttr.equals(userOrgStoreInfo.org().getId().toString())) {
			return false;
		}
