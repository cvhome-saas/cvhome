...
/**
 * A {@link ReactiveJwtDecoder} that supports multiple JWT issuers.
 * It lazily initializes and caches a specific {@link ReactiveJwtDecoder}
 * for each issuer based on the 'iss' claim in the JWT.
 */
public class MultiIssuerReactiveJwtDecoder implements ReactiveJwtDecoder {
     ...
     *
     * @param supportedIssuerUris A set of trusted issuer URIs that this decoder will handle.
     *                            An attempt to decode a token from an issuer not in this set will result in an error.
     *                            Must not be null.
     * @param decoderFactory      A function that takes an issuer URI (String) and returns a
     *                            {@link ReactiveJwtDecoder} configured for that issuer.
     *                            This factory is invoked lazily when a token from a new, supported issuer
     *                            is encountered, and its result is cached. Must not be null.
     *                            The factory should throw an exception if it cannot create a decoder
     *                            for a given supported issuer URI.
     */
    public MultiIssuerReactiveJwtDecoder(
            Set<String> supportedIssuerUris, Function<String, ReactiveJwtDecoder> decoderFactory) {
     ...
     * @return A {@link Mono} emitting the {@link ReactiveJwtDecoder} or an error if the issuer is unsupported
    ...
    private Mono<ReactiveJwtDecoder> getDecoderForIssuer(String issuer) {
        ...
        if (delegateDecoder == null) {
            return Mono.error(
                    new JwtException(
                            String.format(
                                    "Decoder factory returned null for supported issuer: '%s'. This"
                                            + " indicates an issue with the factory configuration.",
                                    issuer)));
        }
     ...
     * @throws JwtException if the token cannot be parsed or the 'iss' claim is missing or empty.
     */
