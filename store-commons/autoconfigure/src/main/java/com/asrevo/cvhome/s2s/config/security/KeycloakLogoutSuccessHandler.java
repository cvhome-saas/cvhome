...
public class KeycloakLogoutSuccessHandler {
    ...
    private static String extractLogoutUrl(
        ...
        String redirectPath =
                Optional.ofNullable(
                                exchange.getRequest().getQueryParams().getFirst("redirect_path"))
                        .orElse("");
        String idTokenHintParam =
                Optional.ofNullable(idToken).map(it -> "&id_token_hint=" + it).orElse("");
        ...
        try {
            if (authentication instanceof OAuth2AuthenticationToken oAuth2AuthenticationToken) {
                ...
                if (clientRegistration != null && clientRegistration.getProviderDetails() != null) {
                    String idToken = null;
                    if (oAuth2AuthenticationToken.getPrincipal()
                            instanceof DefaultOidcUser defaultOidcUser) {
                        idToken = defaultOidcUser.getIdToken().getTokenValue();
                    }
                    String logoutUrl =
                            extractLogoutUrl(
                                    exchange, clientRegistration.getProviderDetails(), idToken);
