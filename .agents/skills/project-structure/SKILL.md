...
Use `./extra/scripts/lcl start -d` to bring the stack up, `lcl status` / `lcl urls` to see it, `lcl restart <svc>` /
`lcl stop <svc>` for one service (the rest keep running), `lcl why <svc>` and `lcl logs <svc> --errors` when
something is wrong, `lcl stop` to tear this instance down. Several named stacks can run at once (`--stack xxx`); when the
configured ports are taken a stack shifts to the next free +1000 sequence (`lcl ports`). `run-lcl.sh` is a shim.
...
| Bring the local stack up or shut it down | `references/qa-testing.md` §1 (`lcl start/stop/restart/status/logs/why`d`) |
