...
| A10 | **L** | payment, tenancy | `payment/controller/v1/auth/AuthController.java`, `tenancy/.../AuthApi.java:41` | Echo the entire JWT to its holder; payment's is outside `/private/`; no consumer | Delete payment's; tenancy keeps `current` behind `isAuthenticated()` | P2, P3 | P2 done (payment), P3 done (tenancy) |
...
- P6b: the branch merges the P2 and P3 fixes so the gate rule is green everywhere; A17 (the two enum reads
  under `/private/` on payment) takes `isAuthenticated()` in the same PR, the honest audience for a list that
  names no store.
